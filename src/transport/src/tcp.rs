...
impl TcpDialer {
    ...
    pub async fn new(ttl: Option<u32>) -> io::Result<Self> {
        Ok(Self { ttl })
    }
    ...
    pub async fn do_dial(
        ...
        debug!(target: "transport::tcp::do_dial", "Dialing {socket_addr} with TCP...");
...
impl TcpListener {
    ...
    pub async fn do_listen(&self, socket_addr: SocketAddr) -> io::Result<SmolTcpListener> {
        let socket = self.create_socket(socket_addr).await?;
        ...
        #[expect(clippy::expect_used, reason = "port is set exactly once per listener")]
