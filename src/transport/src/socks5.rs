...
impl Socks5Dialer {
    ...
    pub async fn new(uri: &Url) -> io::Result<Self> {
    ...
    pub async fn do_dial(&self) -> io::Result<TcpStream> {
        debug!(
            target: "transport::socks5::do_dial",
...
impl Socks5Client {
    ...
    pub async fn connect(&self, addr: impl Into<AddrKind> + Debug) -> io::Result<TcpStream> {
        ...
        stream.flush().await?;
        debug!(
            target: "transport::transport::socks5::connect",
