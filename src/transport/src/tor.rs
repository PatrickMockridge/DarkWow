...
impl TorDialer {
    ...
    pub async fn new(datastore: Option<PathBuf>) -> io::Result<Self> {
        // Initialize or fetch the static TOR_CLIENT that should be reused in
        // the Tor dialer
        let client = match TOR_CLIENT
            .get_or_try_init(|| async {
                debug!(target: "transport::tor::TorDialer", "Bootstrapping...");
            ...
            .await
        {
            ...
            Err(e) => {
                warn!(target: "transport::tor::TorDialer", "{}", e.report());
    ...
    pub async fn do_dial(
        ...
        debug!(target: "transport::tor::do_dial", "Dialing {host}:{port} with Tor...");
        ...
        match conn_timeout {
            Some(t) => {
                ...
                match select(connect, timeout).await {
                    ...
                    Either::Left((Err(e), _)) => {
                        warn!(target: "transport::tor::do_dial", "{}", e.report());
...
impl TorListener {
    ...
    pub async fn do_listen(&self, port: u16) -> io::Result<TorListenerIntern> {
        // Initialize or fetch the static TOR_CLIENT that should be reused in
        // the Tor dialer
        let client = match TOR_CLIENT
            .get_or_try_init(|| async {
                debug!(target: "transport::tor::do_listen", "Bootstrapping...");
            ...
            .await
        {
            Ok(client) => client.isolated_client(),
            Err(e) => {
                warn!(target: "transport::tor::do_listen", "{}", e.report());
                return Err(io::Error::other("Internal Tor error, see logged warning"));
            }
        };
...
impl PtListener for TorListenerIntern {
    async fn next(&self) -> io::Result<(Box<dyn PtStream>, Url)> {
        ...
        let mut streamreq_stream = match rendrequest.accept().await {
            ...
            Err(e) => {
                error!(
                    target: "transport::tor::PtListener::next",
