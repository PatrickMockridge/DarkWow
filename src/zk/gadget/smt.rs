...
// ---- canonical-position enforcement (< p) ----------------------------------
// Without this gate, a malicious prover can witness the non-canonical
// decomposition of N+p instead of N, walk to a different SMT leaf, and
// forge exclusion proofs. See the smt_exclusion_forgery test.
...
impl PathChip {
    pub fn configure(
        ...
        // Path builder: next = 2*current + bit
        ...
        // Boolean constraint for auxiliary witnessed bit (used for k range-check)
        ...
        // Canonicity of the position decomposition: enforces pos < p.
        // The 255-bit position must be strictly less than the Pallas modulus.
        // Without this gate, a prover can substitute N+p for N (they are equal
        // modulo p but have different bit decompositions), walk to a different
        // SMT leaf, and forge exclusion proofs (double-spend).
        meta.create_gate("canonical SMT position (< p)", |meta| {
            ...
            Constraints::with_selector(
                s_canon,
                [
                    // a) pos == high * 2^128 + low
                    high.clone() * two_128 + low.clone() - pos,
                    // b) bit254 == 1  =>  high == 2^126
                    top.clone() * (high - two_126),
                    // c) bit254 == 1  =>  low + k == T-1  (k in [0, 2^128))
                    top * (low + k - t_minus_1),
                ],
            )
        });
    ...
    /// Core membership logic. `bits_value` is the little-endian position
    /// decomposition from the prover. The canonicity gate ensures the
    /// decomposition is the canonical one (< p).
    fn membership_inner(
        &self,
        layouter: &mut impl Layouter<Fp>,
        pos: AssignedCell<Fp, Fp>,
        ...
        // Force canonical position decomposition
    ...
    /// little-endian decomposition. Rejects non-canonical `pos+p`.
    ...
    /// Running sum of bits: accumulates `sum(bits[i] * 2^i)` using the
    /// path-builder gate.
    ...
    /// Witness `value` as `n_bits` fresh, boolean-constrained bits.
    fn range_check_value(
        ...
        for i in (0..n_bits).rev() {
            let bit_val = value.map(|v| {
                ...
                if le[i] { Fp::ONE } else { Fp::ZERO }
            });

            acc = layouter.assign_region(
                || "range-check: acc = 2*acc + bit",
                |mut region| {
                    self.config.s_path.enable(&mut region, 0)?;
    ...
    /// Test-only variant that accepts explicit bits (for forgery test).
    #[cfg(test)]
...
mod tests {
    ...
    /// Without the canonicity gate, a prover can substitute N+p for N,
    /// walk to an empty leaf, and forge an exclusion proof (double-spend).
    fn smt_exclusion_forgery() {
        use dwow_sdk::crypto::pasta_prelude::PrimeField;
        use dwow_sdk::crypto::smt::util::FieldHasher;
        use dwow_sdk::crypto::smt::StorageAdapter;
        use dwow_sdk::num_bigint::BigUint;
        ...
        fn le_bits(x: &BigUint) -> Vec<Fp> {
            (0..SMT_FP_DEPTH)
                .map(|i| if x.bit(i as u64) { Fp::ONE } else { Fp::ZERO })
                .collect()
        }
        ...
        // Build the nullifier tree and "spend" N (inserted as (N, N)).
        // N is in the LEFT half (bit 254 == 0) so N+p lands in the empty
        // RIGHT half and fits in 255 bits.
        ...
        // The forged bits still sum to N over the field, so assert_equal passes.
        // Only the canonicity gate rejects them.
        ...
        // Forge the auth path for the empty leaf at index N+p.
        let left_subtree_root = smt
            .store
            .get(&BigUint::from(1u32))
            .expect("left subtree root must be stored");
        ...
        // Non-vacuousness anchor: outside the circuit, the forged witness
        // DOES authenticate an EMPTY leaf against the genuine root. Without
