...
impl Attestation {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let cd = SerializedLen::try_from_len(self.claim_data.len())?;
        let md = SerializedLen::try_from_len(self.metadata.len())?;
        let cap = 114 + self.claim_data.len() * 32 + self.metadata.len();
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 116 { return Err(ContractError::IoError(format!("Attestation: expected at least 116 bytes, got {}", data.len()))); }
        ...
        let cd_count = SerializedLen::from_le_bytes(read_field::<4>(data, 98)?).to_usize();
        let cd_end = 102 + cd_count * 32;
        if data.len() < cd_end + 4 { return Err(ContractError::IoError("Attestation: data too short for claim_data".into())); }
        let mut claim_data = Vec::with_capacity(cd_count);
        for i in 0..cd_count { claim_data.push(Option::<pallas::Base>::from(pallas::Base::from_repr(read_field::<32>(data, 102 + i*32)?)).ok_or_else(|| ContractError::IoError(format!("Attestation: invalid claim_data[{}]", i)))?); }
        let md_len = SerializedLen::from_le_bytes(read_field::<4>(data, cd_end)?).to_usize();
        let md_end = cd_end + 4 + md_len;
        if data.len() < md_end + 10 { return Err(ContractError::IoError("Attestation: data too short for metadata+state".into())); }
        let metadata = read_slice(data, cd_end + 4, md_end - (cd_end + 4))?.to_vec();
...
impl Claim {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let ec = SerializedLen::try_from_len(self.evidence_commitment.len())?;
        let rr = SerializedLen::try_from_len(self.revealed_result.len())?;
        let pr = SerializedLen::try_from_len(self.proof.len())?;
        ...
        b.extend_from_slice(&rr.to_le_bytes());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let ec_len = SerializedLen::from_le_bytes(read_field::<4>(data, 130)?).to_usize();
        ...
        let rr_len = SerializedLen::from_le_bytes(read_field::<4>(data, ec_end)?).to_usize();
        ...
        let pr_len = SerializedLen::from_le_bytes(read_field::<4>(data, rr_end)?).to_usize();
...
impl dwow_serial::Encodable for CreateAttestationParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl CreateAttestationParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let cd = SerializedLen::try_from_len(self.claim_data.len())?; let md = SerializedLen::try_from_len(self.metadata.len())?; let mut b = Vec::with_capacity(77+self.proof.len()+self.claim_data.len()*32+self.metadata.len()); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); b.extend_from_slice(&self.attestation_id.to_bytes()); b.extend_from_slice(&self.attestor_pub.to_bytes()); b.extend_from_slice(&self.claim_type.encode()); b.extend_from_slice(&cd.to_le_bytes()); for d in &self.claim_data { b.extend_from_slice(&d.to_repr()); } b.extend_from_slice(&md.to_le_bytes()); b.extend_from_slice(&self.metadata); b.push(self.expires_at.is_some() as u8); if let Some(e) = self.expires_at { b.extend_from_slice(&e.to_le_bytes()); } Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 77 { return Err(ContractError::IoError("CreateAttestationParamsV1: too short".into())); } let proof_len = SerializedLen::from_le_bytes(read_field::<4>(data, 0)?).to_usize(); let mut pos = 4+proof_len; if data.len() < pos+64+1 { return Err(ContractError::IoError("CreateAttestationParamsV1: truncated".into())); } let proof = read_slice(data, 4, (pos) - (4))?.to_vec(); let attestation_id = AttestationId::from_bytes(&read_field::<32>(data, pos)?).ok_or_else(|| ContractError::IoError("CreateAttestationParamsV1: invalid attestation_id".into()))?; pos += 32; let attestor_pub = PublicKey::from_bytes(read_field::<32>(data, pos)?).map_err(|e| ContractError::IoError(format!("CreateAttestationParamsV1: invalid attestor_pub: {}", e)))?; pos += 32; let claim_type = Predicate::decode(read_slice(data, pos, 1)?)?; pos += 1; let cd_count = SerializedLen::from_le_bytes(read_field::<4>(data, pos)?).to_usize(); pos += 4; if data.len() < pos+cd_count*32+4 { return Err(ContractError::IoError("CreateAttestationParamsV1: claim_data truncated".into())); } let mut claim_data = Vec::with_capacity(cd_count); for i in 0..cd_count { claim_data.push(Option::<pallas::Base>::from(pallas::Base::from_repr(read_field::<32>(data, pos+i*32)?)).ok_or_else(|| ContractError::IoError(format!("CreateAttestationParamsV1: invalid claim_data[{}]", i)))?); } pos += cd_count*32; let md_len = SerializedLen::from_le_bytes(read_field::<4>(data, pos)?).to_usize(); pos += 4; if data.len() < pos+md_len+1 { return Err(ContractError::IoError("CreateAttestationParamsV1: metadata truncated".into())); } let metadata = read_slice(data, pos, md_len)?.to_vec(); pos += md_len; let has_expiry = read_byte(data, pos)? != 0; let expires_at = if has_expiry { if data.len() != pos+9 { return Err(ContractError::IoError(format!("CreateAttestationParamsV1: expected {} bytes, got {}", pos+9, data.len()))); } Some(u64::from_le_bytes(read_field::<8>(data, pos+1)?)) } else { None }; Ok(CreateAttestationParamsV1 { proof, attestation_id, attestor_pub, claim_type, claim_data, metadata, expires_at }) } }
...
impl dwow_serial::Encodable for CreateAttestationUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for CreateClaimParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl CreateClaimParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let ec = SerializedLen::try_from_len(self.evidence_commitment.len())?; let rr = SerializedLen::try_from_len(self.revealed_result.len())?; let mut b = Vec::with_capacity(108+self.proof.len()+self.evidence_commitment.len()+self.revealed_result.len()); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); b.extend_from_slice(&self.claim_id.to_bytes()); b.extend_from_slice(&self.attestation_id.to_bytes()); b.extend_from_slice(&self.claimant_pub.to_bytes()); b.extend_from_slice(&self.predicate.encode()); b.extend_from_slice(&ec.to_le_bytes()); b.extend_from_slice(&self.evidence_commitment); b.extend_from_slice(&rr.to_le_bytes()); b.extend_from_slice(&self.revealed_result); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 102 { return Err(ContractError::IoError("CreateClaimParamsV1: too short".into())); } let proof_len = SerializedLen::from_le_bytes(read_field::<4>(data, 0)?).to_usize(); let mut pos = 4+proof_len; if data.len() < pos+96+4 { return Err(ContractError::IoError("CreateClaimParamsV1: truncated".into())); } let proof = read_slice(data, 4, (pos) - (4))?.to_vec(); let claim_id = ClaimId::from_bytes(&read_field::<32>(data, pos)?).ok_or_else(|| ContractError::IoError("CreateClaimParamsV1: invalid claim_id".into()))?; pos += 32; let attestation_id = AttestationId::from_bytes(&read_field::<32>(data, pos)?).ok_or_else(|| ContractError::IoError("CreateClaimParamsV1: invalid attestation_id".into()))?; pos += 32; let claimant_pub = PublicKey::from_bytes(read_field::<32>(data, pos)?).map_err(|e| ContractError::IoError(format!("CreateClaimParamsV1: invalid claimant_pub: {}", e)))?; pos += 32; let predicate = Predicate::decode(read_slice(data, pos, 1)?)?; pos += 1; if data.len() < pos+4 { return Err(ContractError::IoError("CreateClaimParamsV1: evidence length truncated".into())); } let ec_len = SerializedLen::from_le_bytes(read_field::<4>(data, pos)?).to_usize(); pos += 4; if data.len() < pos+ec_len+4 { return Err(ContractError::IoError("CreateClaimParamsV1: evidence truncated".into())); } let evidence_commitment = read_slice(data, pos, ec_len)?.to_vec(); pos += ec_len; if data.len() < pos+4 { return Err(ContractError::IoError("CreateClaimParamsV1: revealed_result length truncated".into())); } let rr_len = SerializedLen::from_le_bytes(read_field::<4>(data, pos)?).to_usize(); pos += 4; if data.len() != pos+rr_len { return Err(ContractError::IoError(format!("CreateClaimParamsV1: expected {} bytes, got {}", pos+rr_len, data.len()))); } let revealed_result = data.get(pos..).ok_or_else(|| ContractError::IoError("payload truncated".to_string()))?.to_vec(); Ok(CreateClaimParamsV1 { proof, claim_id, attestation_id, claimant_pub, predicate, evidence_commitment, revealed_result }) } }
...
impl dwow_serial::Encodable for CreateClaimUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for VerifyClaimUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for ValidateClaimParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl ValidateClaimParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.evidence.len())?; let mut b = Vec::with_capacity(68+self.evidence.len()*32); b.extend_from_slice(&self.claim_id.to_bytes()); b.extend_from_slice(&self.attestation_id.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); for e in &self.evidence { b.extend_from_slice(&e.to_repr()); } Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 68 { return Err(ContractError::IoError("ValidateClaimParamsV1: too short".into())); } let claim_id = ClaimId::from_bytes(&read_field::<32>(data, 0)?).ok_or_else(|| ContractError::IoError("ValidateClaimParamsV1: invalid claim_id".into()))?; let attestation_id = AttestationId::from_bytes(&read_field::<32>(data, 32)?).ok_or_else(|| ContractError::IoError("ValidateClaimParamsV1: invalid attestation_id".into()))?; let count = SerializedLen::from_le_bytes(read_field::<4>(data, 64)?).to_usize(); if data.len() != 68+count*32 { return Err(ContractError::IoError(format!("ValidateClaimParamsV1: expected {} bytes, got {}", 68+count*32, data.len()))); } let mut evidence = Vec::with_capacity(count); for i in 0..count { evidence.push(Option::<pallas::Base>::from(pallas::Base::from_repr(read_field::<32>(data, 68+i*32)?)).ok_or_else(|| ContractError::IoError(format!("ValidateClaimParamsV1: invalid evidence[{}]", i)))?); } Ok(ValidateClaimParamsV1 { claim_id, attestation_id, evidence }) } }
...
impl dwow_serial::Encodable for DelegateAttestationParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for DelegateAttestationUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for CheckNotRevokedParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl CheckNotRevokedParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let mut b = Vec::with_capacity(68+self.proof.len()); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); b.extend_from_slice(&self.revocation_root.to_repr()); b.extend_from_slice(&self.nonce.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 68 { return Err(ContractError::IoError("CheckNotRevokedParamsV1: too short".into())); } let proof_len = SerializedLen::from_le_bytes(read_field::<4>(data, 0)?).to_usize(); let pos = 4+proof_len; if data.len() != pos+64 { return Err(ContractError::IoError(format!("CheckNotRevokedParamsV1: expected {} bytes, got {}", pos+64, data.len()))); } let proof = read_slice(data, 4, (pos) - (4))?.to_vec(); let revocation_root = Option::<pallas::Base>::from(pallas::Base::from_repr(read_field::<32>(data, pos)?)).ok_or_else(|| ContractError::IoError("CheckNotRevokedParamsV1: invalid revocation_root".into()))?; let nonce = Option::<pallas::Base>::from(pallas::Base::from_repr(read_field::<32>(data, pos+32)?)).ok_or_else(|| ContractError::IoError("CheckNotRevokedParamsV1: invalid nonce".into()))?; Ok(CheckNotRevokedParamsV1 { proof, revocation_root, nonce }) } }
...
impl dwow_serial::Encodable for VerifyChainParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for UpdateDelegationUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for CommitFeeScheduleParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl CommitFeeScheduleParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let md = SerializedLen::try_from_len(self.metadata.len())?; let mut b = Vec::with_capacity(68+self.metadata.len()); b.extend_from_slice(&self.attestor_pub.to_bytes()); b.extend_from_slice(&self.base_fee_bp.to_le_bytes()); b.extend_from_slice(&self.guaranteed_premium_bp.to_le_bytes()); b.extend_from_slice(&self.max_amount.to_le_bytes()); b.extend_from_slice(&self.min_amount.to_le_bytes()); b.extend_from_slice(&md.to_le_bytes()); b.extend_from_slice(&self.metadata); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 68 { return Err(ContractError::IoError("CommitFeeScheduleParamsV1: too short".into())); } let attestor_pub = PublicKey::from_bytes(read_field::<32>(data, 0)?).map_err(|e| ContractError::IoError(format!("CommitFeeScheduleParamsV1: invalid attestor_pub: {}", e)))?; let base_fee_bp = u64::from_le_bytes(read_field::<8>(data, 32)?); let guaranteed_premium_bp = u64::from_le_bytes(read_field::<8>(data, 40)?); let max_amount = u64::from_le_bytes(read_field::<8>(data, 48)?); let min_amount = u64::from_le_bytes(read_field::<8>(data, 56)?); let md_len = SerializedLen::from_le_bytes(read_field::<4>(data, 64)?).to_usize(); if data.len() != 68+md_len { return Err(ContractError::IoError(format!("CommitFeeScheduleParamsV1: expected {} bytes, got {}", 68+md_len, data.len()))); } let metadata = data.get(68..).ok_or_else(|| ContractError::IoError("payload truncated".to_string()))?.to_vec(); Ok(CommitFeeScheduleParamsV1 { attestor_pub, base_fee_bp, guaranteed_premium_bp, max_amount, min_amount, metadata }) } }
...
impl dwow_serial::Encodable for RevokeAttestationUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for ExpireAttestationUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for ConsumeClaimUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl dwow_serial::Encodable for AttestSlashUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl AttestSlashUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        ...
        let ikb = SerializedLen::try_from_len(self.index_key_bytes.len())?;
        let mut b = Vec::with_capacity(85 + att.len() + self.index_key_bytes.len());
        b.extend_from_slice(&self.attestation_id.to_bytes());
        ...
        b.extend_from_slice(&ikb.to_le_bytes());
        b.extend_from_slice(&self.index_key_bytes);
        b.push(self.is_new as u8);
        Ok(b)
    }
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 85 { return Err(ContractError::IoError(format!("AttestSlashUpdateV1: expected at least 85 bytes, got {}", data.len()))); }
        ...
        if data.len() < ikb_pos + 5 { return Err(ContractError::IoError("AttestSlashUpdateV1: data too short".into())); }
        let ikb_len = SerializedLen::from_le_bytes(read_field::<4>(data, ikb_pos)?).to_usize();
        if data.len() != ikb_pos + 5 + ikb_len { return Err(ContractError::IoError(format!("AttestSlashUpdateV1: size mismatch"))); }
        let index_key_bytes = read_slice(data, ikb_pos + 4, ikb_len)?.to_vec();
        let is_new = read_byte(data, ikb_pos + 4 + ikb_len)? != 0;
        Ok(AttestSlashUpdateV1 { attestation_id, slash_amount, withdrawal_id, block_height, attestation, index_key_bytes, is_new })
    }
}

impl dwow_serial::Encodable for CommitFeeScheduleUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
impl CommitFeeScheduleUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let inner = self.attestation.encode()?;
        let ikb = SerializedLen::try_from_len(self.index_key_bytes.len())?;
        let mut b = Vec::with_capacity(68 + inner.len() + self.index_key_bytes.len());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 68 { return Err(ContractError::IoError(format!("CommitFeeScheduleUpdateV1: expected at least 68 bytes, got {}", data.len()))); }
        ...
        if data.len() < ikb_pos + 4 { return Err(ContractError::IoError("CommitFeeScheduleUpdateV1: data too short for index_key".into())); }
        let ikb_len = SerializedLen::from_le_bytes(read_field::<4>(data, ikb_pos)?).to_usize();
        if data.len() != ikb_pos + 4 + ikb_len { return Err(ContractError::IoError(format!("CommitFeeScheduleUpdateV1: index_key len mismatch, expected {} + 4 + {}", ikb_pos, ikb_len))); }
        let index_key_bytes = data.get(ikb_pos + 4..).ok_or_else(|| ContractError::IoError("payload truncated".to_string()))?.to_vec();
