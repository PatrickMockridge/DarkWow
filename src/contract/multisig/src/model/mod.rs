...
impl MultiSigGroup {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let pk_count = read_byte(data, 33)? as usize;
        ...
        let threshold = read_byte(data, 34 + pk_count * 32)?;
        let total_keys = read_byte(data, 35 + pk_count * 32)?;
...
impl CreateGroupUpdateV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(35+self.pubkeys.len()*32); b.extend_from_slice(&self.group_id.encode()); b.push(self.pubkeys.len() as u8); for pk in &self.pubkeys { b.extend_from_slice(&pk.to_bytes()); } b.push(self.threshold); b.push(self.total_keys); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 35 { return Err(ContractError::IoError("CreateGroupUpdateV1: too short".into())); } let group_id = GroupId::decode(read_slice(data, 0, 32)?)?; let pk_count = read_byte(data, 32)? as usize; let expected = 35+pk_count*32; if data.len() != expected { return Err(ContractError::IoError(format!("CreateGroupUpdateV1: expected {} bytes, got {}", expected, data.len()))); } let mut pubkeys = Vec::with_capacity(pk_count); for i in 0..pk_count { pubkeys.push(PublicKey::from_bytes(read_field::<32>(data, 33+i*32)?).map_err(|e| ContractError::IoError(format!("CreateGroupUpdateV1: invalid pubkey[{}]: {}", i, e)))?); } Ok(CreateGroupUpdateV1 { group_id, pubkeys, threshold: read_byte(data, 33+pk_count*32)?, total_keys: read_byte(data, 34+pk_count*32)? }) } }
