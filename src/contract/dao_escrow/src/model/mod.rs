...
pub struct InitializeUpdateV1 {
    ...
    /// Bulla blind factor
    pub bulla_blind: BaseBlind,
    /// Block height the endowment is stamped with, read in exec. `apply_initialize_update` used to
    /// call `get_verifying_block_height` itself, which the ACL denies in `Update` (register
    /// OBL-C72).
    pub created_at: u64,
}
...
pub struct EnableDrainProtectionUpdateV1 {
    ...
    pub drain_protection_bulla: DaoEscrowBulla,
    /// `DaoEscrow::encode()` with `drain_protection_bulla` set, as exec produced it (OBL-C72).
    pub endowment_bytes: Vec<u8>,
}
...
pub struct VoteClaimUpdateV1 {
    ...
    pub expired: bool,
    /// The voter's capability nullifier, carried here so **apply** marks it spent.
    ///
    ...
    /// `Proposal::encode()` with the new tally applied, as exec produced it. Carried so apply
...
pub struct DeactivateCapabilityRequirementUpdateV1 {
    pub dao_escrow_bulla: DaoEscrowBulla,
    pub role: Vec<u8>,
    /// `CapabilityRequirement::encode()` with the requirement deactivated, as exec produced it
    /// (register OBL-C72). Carried because apply may not read the record back to modify it.
...
impl dwow_serial::Encodable for WithdrawParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl RegisterCapabilityRequirementParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.role.len())?; let mut b = Vec::with_capacity(68+self.role.len()); b.extend_from_slice(&self.dao_escrow_bulla.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.role); b.extend_from_slice(&self.capability_id); b.extend_from_slice(&self.identity_contract_bulla.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 68 { return Err(ContractError::IoError("RegisterCapabilityRequirementParamsV1: too short".into())); } let role_len = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize(); let expected = role_len.saturating_add(68); if data.len() != expected { return Err(ContractError::IoError(format!("RegisterCapabilityRequirementParamsV1: expected {} got {}", expected, data.len()))); } Ok(RegisterCapabilityRequirementParamsV1 { dao_escrow_bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("RegisterCapabilityRequirementParamsV1: invalid dao_escrow_bulla".into()))?), role: data[36..36+role_len].to_vec(), capability_id: data[36+role_len..68+role_len].try_into().unwrap(), identity_contract_bulla: Option::<pallas::Base>::from(pallas::Base::from_repr(data[68+role_len..100+role_len].try_into().unwrap())).ok_or_else(|| ContractError::IoError("RegisterCapabilityRequirementParamsV1: invalid identity_contract_bulla".into()))? }) } }

#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl VerifyMemberCapabilityParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let cp = self.capability_proof.encode()?; let mut b = Vec::with_capacity(32+cp.len()+32); b.extend_from_slice(&self.dao_escrow_bulla.to_bytes()); b.extend_from_slice(&cp); b.extend_from_slice(&self.holder_pubkey.to_bytes()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 64 { return Err(ContractError::IoError("VerifyMemberCapabilityParamsV1: too short".into())); } let dao_escrow_bulla = DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("VerifyMemberCapabilityParamsV1: invalid dao_escrow_bulla".into()))?); let holder_pubkey = PublicKey::from_bytes(data[data.len()-32..].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("VerifyMemberCapabilityParamsV1: invalid holder_pubkey: {}", e)))?; let capability_proof = CapabilityProof::decode(&data[32..data.len()-32])?; Ok(VerifyMemberCapabilityParamsV1 { dao_escrow_bulla, capability_proof, holder_pubkey }) } }

#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl DeactivateCapabilityRequirementParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.role.len())?; let mut b = Vec::with_capacity(36+self.role.len()); b.extend_from_slice(&self.dao_escrow_bulla.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.role); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 36 { return Err(ContractError::IoError("DeactivateCapabilityRequirementParamsV1: too short".into())); } let role_len = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize(); let expected = role_len.saturating_add(36); if data.len() != expected { return Err(ContractError::IoError(format!("DeactivateCapabilityRequirementParamsV1: expected {} got {}", expected, data.len()))); } Ok(DeactivateCapabilityRequirementParamsV1 { dao_escrow_bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("DeactivateCapabilityRequirementParamsV1: invalid dao_escrow_bulla".into()))?), role: data[36..36+role_len].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl CapabilityProof { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let mut b = Vec::with_capacity(164+self.proof.len()); b.extend_from_slice(&self.capability_id); b.extend_from_slice(&self.capability_secret); b.extend_from_slice(&self.nullifier.to_bytes()); b.extend_from_slice(&self.issuer_pub); b.extend_from_slice(&self.predicate_result); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 164 { return Err(ContractError::IoError("CapabilityProof: too short".into())); } let proof_len = SerializedLen::from_le_bytes(data[160..164].try_into().unwrap()).to_usize(); let expected = proof_len.saturating_add(164); if data.len() != expected { return Err(ContractError::IoError(format!("CapabilityProof: expected {} got {}", expected, data.len()))); } Ok(CapabilityProof { capability_id: data[0..32].try_into().unwrap(), capability_secret: data[32..64].try_into().unwrap(), nullifier: IntentNullifier::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("CapabilityProof: invalid nullifier: {}", e)))?, issuer_pub: data[96..128].try_into().unwrap(), predicate_result: data[128..160].try_into().unwrap(), proof: data[164..164+proof_len].to_vec() }) } }
...
#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl InitializeUpdateV1 { pub const ENCODED_SIZE: usize = 136; pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(136); b.extend_from_slice(&self.instance_seed); b.extend_from_slice(&self.bulla.to_bytes()); b.extend_from_slice(&self.owner_pubkey.to_bytes()); b.extend_from_slice(&self.bulla_blind.inner().to_repr()); b.extend_from_slice(&self.created_at.to_le_bytes()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 136 { return Err(ContractError::IoError(format!("InitializeUpdateV1: expected 136 bytes, got {}", data.len()))); } Ok(InitializeUpdateV1 { instance_seed: data[0..32].try_into().unwrap(), bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("InitializeUpdateV1: invalid bulla".into()))?), owner_pubkey: PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("InitializeUpdateV1: invalid owner_pubkey: {}", e)))?, bulla_blind: dwow_sdk::crypto::Blind(Option::<pallas::Base>::from(pallas::Base::from_repr(data[96..128].try_into().unwrap())).ok_or_else(|| ContractError::IoError("InitializeUpdateV1: invalid bulla_blind".into()))?), created_at: u64::from_le_bytes(data[128..136].try_into().unwrap()) }) } }
...
#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl TreasurySpendUpdateV1 { pub const FIXED: usize = 84; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = dwow_sdk::blockchain::SerializedLen::try_from_len(self.endowment_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.endowment_bytes.len()); b.extend_from_slice(&self.dao_escrow_bulla.to_bytes()); b.extend_from_slice(&self.proposal_id.to_repr()); b.extend_from_slice(&self.value.to_le_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.endowment_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("TreasurySpendUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = dwow_sdk::blockchain::SerializedLen::from_le_bytes(data[80..84].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("TreasurySpendUpdateV1: {} endowment bytes do not fit {} total", n, data.len()))); } Ok(TreasurySpendUpdateV1 { dao_escrow_bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("TreasurySpendUpdateV1: invalid dao_escrow_bulla".into()))?), proposal_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("TreasurySpendUpdateV1: invalid proposal_id".into()))?, value: u64::from_le_bytes(data[64..72].try_into().unwrap()), amount: u64::from_le_bytes(data[72..80].try_into().unwrap()), endowment_bytes: data[Self::FIXED..].to_vec() }) } }

impl dwow_serial::Encodable for EnableDrainProtectionUpdateV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl EnableDrainProtectionUpdateV1 { pub const FIXED: usize = 68; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = dwow_sdk::blockchain::SerializedLen::try_from_len(self.endowment_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.endowment_bytes.len()); b.extend_from_slice(&self.dao_escrow_bulla.to_bytes()); b.extend_from_slice(&self.drain_protection_bulla.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.endowment_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("EnableDrainProtectionUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = dwow_sdk::blockchain::SerializedLen::from_le_bytes(data[64..68].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("EnableDrainProtectionUpdateV1: {} endowment bytes do not fit {} total", n, data.len()))); } Ok(EnableDrainProtectionUpdateV1 { dao_escrow_bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("EnableDrainProtectionUpdateV1: invalid dao_escrow_bulla".into()))?), drain_protection_bulla: DaoEscrowBulla(Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("EnableDrainProtectionUpdateV1: invalid drain_protection_bulla".into()))?), endowment_bytes: data[Self::FIXED..].to_vec() }) } }
...
impl CapabilityRequirement {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let n = SerializedLen::try_from_len(self.role.len())?;
        let mut b = Vec::with_capacity(70 + self.role.len());
        b.push(self.version);
    ...
    #[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        // version(1) + role length(4) + capability_id(32) + identity_contract_bulla(32) + active(1)
        if data.len() < 70 { return Err(ContractError::IoError(format!("CapabilityRequirement: expected at least 70 bytes, got {}", data.len()))); }
        let version = data[0];
        let role_len = SerializedLen::from_le_bytes(data[1..5].try_into().unwrap()).to_usize();
        let expected = role_len.saturating_add(70);
        if data.len() != expected { return Err(ContractError::IoError(format!("CapabilityRequirement: expected {} bytes, got {}", expected, data.len()))); }
        let role = data[5..5+role_len].to_vec();
        let capability_id: [u8; 32] = data[5+role_len..37+role_len].try_into().unwrap();
        let identity_contract_bulla = Option::<pallas::Base>::from(pallas::Base::from_repr(data[37+role_len..69+role_len].try_into().unwrap())).ok_or_else(|| ContractError::IoError("CapabilityRequirement: invalid identity_contract_bulla".into()))?;
        let active = data[69+role_len] != 0;
...
impl RegisterCapabilityRequirementUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        ...
        let n = SerializedLen::try_from_len(self.role.len())?;
        let mut b = Vec::with_capacity(36 + self.role.len() + inner.len());
        b.extend_from_slice(&self.dao_escrow_bulla.to_bytes());
        b.extend_from_slice(&n.to_le_bytes());
        b.extend_from_slice(&self.role);
    ...
    #[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 36 { return Err(ContractError::IoError(format!("RegisterCapabilityRequirementUpdateV1: expected at least 36 bytes, got {}", data.len()))); }
        ...
        let role_len = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize();
        if data.len() < role_len.saturating_add(36) { return Err(ContractError::IoError("RegisterCapabilityRequirementUpdateV1: data too short".into())); }
        let role = data[36..36+role_len].to_vec();
        let requirement = CapabilityRequirement::decode(&data[36+role_len..])?;
...
impl ResolveDisputeUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let mut b = Vec::with_capacity(109 + self.consumed_attestation_ids.len() * 32);
        b.extend_from_slice(&self.dao_escrow_bulla.to_bytes());
        ...
        b.extend_from_slice(&SerializedLen::try_from_len(self.consumed_attestation_ids.len())?.to_le_bytes());
        for id in &self.consumed_attestation_ids { b.extend_from_slice(&id.to_repr()); }
        Ok(b)
    }
    #[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 109 { return Err(ContractError::IoError(format!("ResolveDisputeUpdateV1: expected at least 109 bytes, got {}", data.len()))); }
        ...
        let count = SerializedLen::from_le_bytes(data[105..109].try_into().unwrap()).to_usize();
        let expected = count.saturating_mul(32).saturating_add(109);
        ...
        for i in 0..count { consumed_attestation_ids.push(Option::<pallas::Base>::from(pallas::Base::from_repr(data[109+i*32..109+(i+1)*32].try_into().unwrap())).ok_or_else(|| ContractError::IoError(format!("ResolveDisputeUpdateV1: invalid attestation_id[{}]", i)))?); }
...
impl DeactivateCapabilityRequirementUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let n = SerializedLen::try_from_len(self.role.len())?;
        let r = SerializedLen::try_from_len(self.requirement_bytes.len())?;
        let mut b = Vec::with_capacity(40 + self.role.len() + self.requirement_bytes.len());
        b.extend_from_slice(&self.dao_escrow_bulla.to_bytes());
        b.extend_from_slice(&n.to_le_bytes());
        b.extend_from_slice(&self.role);
        b.extend_from_slice(&r.to_le_bytes());
        b.extend_from_slice(&self.requirement_bytes);
        Ok(b)
    }
    #[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 40 { return Err(ContractError::IoError(format!("DeactivateCapabilityRequirementUpdateV1: expected at least 40 bytes, got {}", data.len()))); }
        ...
        let role_len = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize();
        let rpos = 36usize.saturating_add(role_len);
        if data.len() < rpos + 4 { return Err(ContractError::IoError("DeactivateCapabilityRequirementUpdateV1: truncated".into())); }
        let req_len = SerializedLen::from_le_bytes(data[rpos..rpos+4].try_into().unwrap()).to_usize();
        let expected = rpos.saturating_add(4).saturating_add(req_len);
        if data.len() != expected { return Err(ContractError::IoError(format!("DeactivateCapabilityRequirementUpdateV1: expected {} bytes, got {}", expected, data.len()))); }
        let role = data[36..rpos].to_vec();
        Ok(DeactivateCapabilityRequirementUpdateV1 { dao_escrow_bulla, role, requirement_bytes: data[rpos+4..].to_vec() })
    }
}
