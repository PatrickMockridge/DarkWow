...
#[expect(clippy::unwrap_used, reason = "internally-consistent serialized data")]
impl CapabilityProof { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(164+self.proof.len()); b.extend_from_slice(&self.capability_id); b.extend_from_slice(&self.capability_secret); b.extend_from_slice(&self.nullifier.to_bytes()); b.extend_from_slice(&self.issuer_pub); b.extend_from_slice(&self.predicate_result); b.extend_from_slice(&(self.proof.len() as u32).to_le_bytes()); b.extend_from_slice(&self.proof); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 164 { return Err(ContractError::IoError("CapabilityProof: too short".into())); } let proof_len = u32::from_le_bytes(data[160..164].try_into().unwrap()) as usize; if data.len() != 164+proof_len { return Err(ContractError::IoError(format!("CapabilityProof: expected {} got {}", 164+proof_len, data.len()))); } Ok(CapabilityProof { capability_id: data[0..32].try_into().unwrap(), capability_secret: data[32..64].try_into().unwrap(), nullifier: IntentNullifier::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("CapabilityProof: invalid nullifier: {}", e)))?, issuer_pub: data[96..128].try_into().unwrap(), predicate_result: data[128..160].try_into().unwrap(), proof: data[164..164+proof_len].to_vec() }) } }
