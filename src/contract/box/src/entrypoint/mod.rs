...
pub fn init_contract(cid: ContractId, _ix: &[u8]) -> ContractResult {
    ...
    if wasm::db::db_lookup(cid, BOX_CONTRACT_BOX_ROOTS_TREE).is_err() { let db = wasm::db::db_init(cid, BOX_CONTRACT_BOX_ROOTS_TREE)?; wasm::db::db_set(db, &EMPTY_BOX_TREE_ROOT, &roots_value_data)?; }
