...
impl PutParams {
    pub fn encode(&self) -> Vec<u8> {
        ...
        let mut b = Vec::with_capacity(193 + path_bytes.len() + self.proof.len());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 193 + 32*32 { return Err(ContractError::IoError("PutParams: too short".into())); }
        ...
        let merkle_root = read_base(&data[192..224])?;
        ...
        for i in 0..32 { merkle_path[i] = read_base(&data[260 + i*32 .. 260 + (i+1)*32])?; }
        let path_end = 260 + 32*32;
        ...
        Ok(PutParams { box_id, old_state_nonce, new_state_nonce, old_contents_commit, new_contents_commit, nullifier, merkle_root, owner, leaf_pos, merkle_path, proof, tx_binding, tx_nonce })
    }
}
...
impl PutUpdate {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        Ok(PutUpdate { nullifier: read_base(&data[0..32])?, new_contents_commit: read_base(&data[32..64])? })
    }
}
...
impl TakeParams {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 129 + 32*32 { return Err(ContractError::IoError("TakeParams: too short".into())); }
        let box_id = BoxId::decode(&data[0..32])?;
        let contents_commit = read_base(&data[32..64])?;
        let state_nonce = read_base(&data[64..96])?;
        let nullifier = read_base(&data[96..128])?;
        let merkle_root = read_base(&data[128..160])?;
        ...
        let tx_binding = read_base(&data[pos2..pos2+32])?;
        let tx_nonce = read_base(&data[pos2+32..pos2+64])?;
        Ok(TakeParams { box_id, contents_commit, state_nonce, nullifier, merkle_root, owner, leaf_pos, merkle_path, proof, tx_binding, tx_nonce })
    }
}
