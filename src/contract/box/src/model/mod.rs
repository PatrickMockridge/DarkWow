use dwow_sdk::{
    crypto::{pasta_prelude::PrimeField, Nullifier},
    error::ContractError,
    pasta::pallas,
};
...
pub struct PutParams {
    ...
    pub nullifier: Nullifier, pub expected_root: pallas::Base, pub new_leaf: pallas::Base,
...
impl PutParams {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let expected_root = read_base(&data[192..224])?; let new_leaf = read_base(&data[224..256])?;
...
#[derive(Debug, Clone)] pub struct PutUpdate { pub nullifier: Nullifier, pub new_leaf: pallas::Base }
...
impl PutUpdate {
    pub fn encode(&self) -> Vec<u8> { let mut v = Vec::with_capacity(64usize); v.extend_from_slice(&self.nullifier.to_bytes()); v.extend_from_slice(&self.new_leaf.to_repr()); v }
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 64 { return Err(ContractError::IoError("PutUpdate: expected 64 bytes".into())); } Ok(PutUpdate{nullifier:read_nullifier(&data[0..32])?, new_leaf:read_base(&data[32..64])?}) }
}
...
pub struct TakeParams {
    ...
    pub nullifier: Nullifier, pub expected_root: pallas::Base,
    pub leaf_pos: u32, pub merkle_path: MerklePath, pub proof: Vec<u8>,
    pub tx_binding: pallas::Base, pub tx_nonce: pallas::Base,
}
...
impl TakeParams {
    pub fn encode(&self) -> Vec<u8> {
        ...
        b.extend_from_slice(&self.expected_root.to_repr()); b.extend_from_slice(&self.leaf_pos.to_le_bytes());
