...
impl PurseId {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> { self.to_bytes().to_vec() }
...
impl Purse {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mut b=Vec::with_capacity(129); b.push(self.version); b.extend_from_slice(&self.purse_id.to_bytes()); b.extend_from_slice(&self.token_commit.to_repr()); b.extend_from_slice(&self.balance_commit.to_bytes()); b.extend_from_slice(&self.owner_commit.to_repr()); b }
...
impl DepositUpdate { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mut v=Vec::with_capacity(64); v.extend_from_slice(&self.nullifier.to_bytes()); v.extend_from_slice(&self.new_leaf.to_repr()); v } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len()!=64 { return Err(ContractError::IoError("DepositUpdate: expected 64 bytes".into())); } Ok(DepositUpdate{nullifier:Nullifier::decode(&data[0..32])?, new_leaf:read_base(&data[32..64])?}) } }
...
impl WithdrawUpdate { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mut v=Vec::with_capacity(64); v.extend_from_slice(&self.nullifier.to_bytes()); v.extend_from_slice(&self.new_leaf.to_repr()); v } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len()!=64 { return Err(ContractError::IoError("WithdrawUpdate: expected 64 bytes".into())); } Ok(WithdrawUpdate{nullifier:Nullifier::decode(&data[0..32])?, new_leaf:read_base(&data[32..64])?}) } }
