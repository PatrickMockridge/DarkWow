use dwow_sdk::{crypto::{pasta_prelude::PrimeField, MerkleNode}, error::ContractError, pasta::{group::GroupEncoding, pallas}};
...
#[derive(Debug, Clone)] pub struct DepositParams {
    ...
    pub state_nonce: pallas::Base, pub nullifier: Nullifier, pub expected_root: MerkleNode, pub new_leaf: MerkleNode,
...
impl DepositParams {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let er=MerkleNode::from_bytes(data[120..152].try_into().unwrap()).ok_or_else(||ContractError::IoError("DepositParams: invalid expected_root".into()))?; let nl=MerkleNode::from_bytes(data[152..184].try_into().unwrap()).ok_or_else(||ContractError::IoError("DepositParams: invalid new_leaf".into()))?;
...
impl BalanceParams {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let dpi=read_base(&data[104..136])?; let er=MerkleNode::from_bytes(data[136..168].try_into().unwrap()).ok_or_else(||ContractError::IoError("BalanceParams: invalid expected_root".into()))?; let tc=read_base(&data[168..200])?;
