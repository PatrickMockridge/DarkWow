...
//! GameRoom Claim ZK proof generation (ClaimV2 circuit).
//!
//! witness (9): room_id, pot_id, winner_pub_x, winner_pub_y, payout_amount, nonce,
//!              tx_commitment, tx_nonce, tx_binding.
//! `claim_id = poseidon_hash(4, pot_id, winner_pub_x, payout_amount, nonce)`.
//! instances (3): claim_id, tx_binding, tx_nonce.
...
impl ClaimPublicInputs {
    pub fn to_vec(&self) -> Vec<pallas::Base> {
        vec![self.claim_id, self.tx_binding, self.tx_nonce]
    }
}
...
impl ClaimCallData {
    pub fn new(room_id: pallas::Base, pot_id: pallas::Base, winner_public: PublicKey, payout_amount: u64, nonce: pallas::Base) -> Self {
    ...
    pub fn compute_public_inputs(&self) -> ClaimPublicInputs {
    ...
    pub fn to_witnesses(&self) -> Vec<Witness> {
        vec![
            ...
            Witness::Base(Value::known(self.winner_pub_x)),
            Witness::Base(Value::known(self.winner_pub_y)),
...
pub fn claim_v1_proof(
    ...
    pk: &ProvingKey,
    input: &ClaimCallData,
) -> Result<(Proof, ClaimPublicInputs)> {
