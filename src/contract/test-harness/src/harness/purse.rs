...
impl PurseHarness {
    pub fn spawn() -> Self {
        let dz = ZkBinary::decode(include_bytes!("../../../purse/proof/deposit.zk.bin"), false).map_err(|e| format!("decode deposit: {e:?}")).unwrap();
        let wz = ZkBinary::decode(include_bytes!("../../../purse/proof/withdraw.zk.bin"), false).map_err(|e| format!("decode withdraw: {e:?}")).unwrap();
        let bz = ZkBinary::decode(include_bytes!("../../../purse/proof/balance.zk.bin"), false).map_err(|e| format!("decode balance: {e:?}")).unwrap();
        let dp = ProvingKey::build(dz.k, &ZkCircuit::new(dwow_core::zk::empty_witnesses(&dz).map_err(|e| format!("empty deposit: {e:?}")).unwrap(), &dz)).map_err(|e| format!("pk deposit: {e:?}")).unwrap();
        let wp = ProvingKey::build(wz.k, &ZkCircuit::new(dwow_core::zk::empty_witnesses(&wz).map_err(|e| format!("empty withdraw: {e:?}")).unwrap(), &wz)).map_err(|e| format!("pk withdraw: {e:?}")).unwrap();
        let bp = ProvingKey::build(bz.k, &ZkCircuit::new(dwow_core::zk::empty_witnesses(&bz).map_err(|e| format!("empty balance: {e:?}")).unwrap(), &bz)).map_err(|e| format!("pk balance: {e:?}")).unwrap();
    ...
    fn build_tree_and_root(leaf: pallas::Base) -> Result<(u32, Vec<MerkleNode>, pallas::Base)> {
        ...
        for (level, sibling) in p.iter().enumerate() {
            current = if (lp as usize) & (1 << level) == 0 { MerkleNode::combine((level as u8).into(), &current, sibling) } else { MerkleNode::combine((level as u8).into(), sibling, &current) };
        }
    ...
    pub fn deposit(&self, amount: u64) -> Result<PurseDepositResult> {
        ...
        let pr=DepositParams{purse_id:dwow_purse_contract::model::PurseId(pid),old_balance:ob,deposit_amount:amount,new_balance:nb,state_nonce:sn,nullifier:dwow_purse_contract::model::Nullifier(nf),expected_root:er,new_leaf:nl,old_commit_x:ocx,old_commit_y:ocy,new_commit_x:ncx,new_commit_y:ncy,leaf_pos:lp,merkle_path:mpa,proof:pb,tx_binding:tb,tx_nonce:tn};
        let mut cd=vec![0x01u8];cd.extend_from_slice(&pr.encode());
    ...
    pub fn withdraw(&self, amount: u64) -> Result<PurseWithdrawResult> {
        ...
        let proof=Proof::create(&self.withdraw_pk,&[c],&pi,OsRng).map_err(|e| format!("Proof::create withdraw: {e:?}"))?;
        let pb:Vec<u8>=dwow_serial::serialize(&proof);
        let mpa:[pallas::Base;32]=p.iter().map(|n|n.inner()).collect::<Vec<_>>().try_into().map_err(|_| "path array".to_string())?;
        let pr=WithdrawParams{purse_id:dwow_purse_contract::model::PurseId(pid),old_balance:ob,withdraw_amount:amount,new_balance:nb,state_nonce:sn,nullifier:dwow_purse_contract::model::Nullifier(nf),expected_root:er,new_leaf:nl,old_commit_x:ocx,old_commit_y:ocy,new_commit_x:ncx,new_commit_y:ncy,leaf_pos:lp,merkle_path:mpa,proof:pb,tx_binding:tb,tx_nonce:tn};
        let mut cd=vec![0x02u8];cd.extend_from_slice(&pr.encode());
        Ok(PurseWithdrawResult{call_data:cd,proof})
    }

    pub fn balance(&self) -> Result<PurseBalanceResult> {
        let dml=pallas::Base::from(5u64);let dss=pallas::Base::from(7u64);let dtc=pallas::Base::from(2u64);let dtb=pallas::Base::from(3u64);let dcc=pallas::Base::from(4u64);
        let os=pallas::Base::from(42u64);let op=poseidon_hash([dss,os]);let pid=pallas::Base::from(1u64);
        ...
        let ol=poseidon_hash([dml,pid,pallas::Base::from(bal),sn]);
        let (lp,p,er)=Self::build_tree_and_root(ol)?;
        let bbl=ScalarBlind::from(1u64);let bc=pedersen_commitment_u64(bal,bbl.clone());
        let bca=bc.to_affine();let bcx=*bca.coordinates().ok_or_else(|| "balance commit identity".to_string())?.x();let bcy=*bca.coordinates().ok_or_else(|| "balance commit identity".to_string())?.y();
        let w=vec![Witness::Base(Value::known(pid)),Witness::Base(Value::known(tid)),Witness::Base(Value::known(pallas::Base::from(bal))),Witness::Scalar(Value::known(bbl.inner())),Witness::Base(Value::known(sn)),Witness::Base(Value::known(dpi)),Witness::Base(Value::known(er)),Witness::Base(Value::known(tcom)),Witness::Base(Value::known(bcx)),Witness::Base(Value::known(bcy)),Witness::Base(Value::known(os)),Witness::Base(Value::known(op)),Witness::Base(Value::known(tblind)),Witness::Uint32(Value::known(lp)),Witness::MerklePath(Value::known(p.clone().try_into().map_err(|_| "path convert".to_string())?)),Witness::Base(Value::known(tc_)),Witness::Base(Value::known(tn_)),Witness::Base(Value::known(tb))];
        let pi=vec![dpi,er,bcx,bcy,tcom,tb,tn_];
        let c=ZkCircuit::new(w,&self.balance_zkbin);
        let proof=Proof::create(&self.balance_pk,&[c],&pi,OsRng).map_err(|e| format!("Proof::create balance: {e:?}"))?;
        let pb:Vec<u8>=dwow_serial::serialize(&proof);
        let mpa:[pallas::Base;32]=p.iter().map(|n|n.inner()).collect::<Vec<_>>().try_into().map_err(|_| "path array".to_string())?;
        let pr=BalanceParams{purse_id:dwow_purse_contract::model::PurseId(pid),token_id:tid,balance:bal,state_nonce:sn,derived_purse_id:dpi,expected_root:er,token_commit:tcom,balance_commit_x:bcx,balance_commit_y:bcy,leaf_pos:lp,merkle_path:mpa,proof:pb,tx_binding:tb,tx_nonce:tn_};
        let mut cd=vec![0x03u8];cd.extend_from_slice(&pr.encode());
