...
//! Relayer Endowment contract test harness
...
use darkfi_relayer_endowment_contract::{
    model::{InitializeParamsV1, DeployCapitalParamsV1, ClaimFeesParamsV1},
    RelayerEndowmentFunction,
};

use super::{Holder, TestHarness};

impl TestHarness {
    /// Deploy the Relayer Endowment WASM contract.
    pub async fn deploy_relayer_endowment(
        &mut self,
        holder: &Holder,
        wasm_bincode: Vec<u8>,
        block_height: u32,
        ...
        debug!(
            target: "test-harness",
            "Deployed relayer_endowment contract: {:?}",
            contract_id
        );
    ...
    /// Create a `RelayerEndowment::InitializeV1` transaction.
    pub async fn relayer_endowment_initialize(
        &mut self,
        holder: &Holder,
    ...
    /// Execute a `RelayerEndowment::InitializeV1` transaction.
    pub async fn execute_relayer_endowment_initialize_tx(
        &mut self,
        holder: &Holder,
        tx: Transaction,
        ...
        wallet.add_transaction("relayer_endowment::initialize", tx, block_height).await?;
    ...
    /// Create a `RelayerEndowment::DeployCapitalV1` transaction.
    pub async fn relayer_endowment_deploy(
        &mut self,
        holder: &Holder,
        contract_id: ContractId,
    ...
    /// Execute a `RelayerEndowment::DeployCapitalV1` transaction.
    pub async fn execute_relayer_endowment_deploy_tx(
        &mut self,
        holder: &Holder,
        tx: Transaction,
        _params: &DeployCapitalParamsV1,
        fee_params: &Option<MoneyFeeParamsV1>,
        block_height: u32,
        append: bool,
    ) -> Result<Vec<OwnCoin>> {
        ...
        wallet.add_transaction("relayer_endowment::deploy", tx, block_height).await?;
