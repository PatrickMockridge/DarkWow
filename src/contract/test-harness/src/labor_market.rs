...
//! Labor Market contract test harness
...
use darkfi_labor_market_contract::{
    model::{CreateJobParamsV1, SubmitDeliverableParamsV1, AcceptJobParamsV1, DisputeParamsV1, RefundParamsV1},
    LaborMarketFunction,
};
...
impl TestHarness {
    /// Deploy the Labor Market WASM contract.
    pub async fn deploy_labor_market(
        &mut self,
        holder: &Holder,
        wasm_bincode: Vec<u8>,
        block_height: u32,
        ...
        debug!(
            target: "test-harness",
            "Deployed labor_market contract: {:?}",
            contract_id
        );
    ...
    /// Create a `LaborMarket::CreateJobV1` transaction.
    pub async fn labor_market_create_job(
        &mut self,
        holder: &Holder,
    ...
    /// Execute a `LaborMarket::CreateJobV1` transaction.
    pub async fn execute_labor_market_create_job_tx(
        &mut self,
        holder: &Holder,
        tx: Transaction,
        _params: &CreateJobParamsV1,
        ...
        wallet.add_transaction("labor_market::create_job", tx, block_height).await?;
