...
//!     init <dao_bulla> <owner_secret> <asset_id> <bulla_blind>
//!   cargo run -p darkfi-contract-test-harness --bin prove_dao_escrow -- \
//!     pay_premium <escrow_bulla> <member_secret> <value> <asset_id> <expiry>
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage:");
        eprintln!("  prove_dao_escrow init <dao_bulla_hex> <owner_secret_hex> <asset_id_hex> <bulla_blind_hex>");
        eprintln!("  prove_dao_escrow pay_premium <escrow_bulla_hex> <member_secret_hex> <value_u64> <asset_id_hex> <expiry_u64>");
        std::process::exit(1);
    }
...
fn run_init(args: &[String]) {
    if args.len() != 4 {
        eprintln!("Usage: init <dao_bulla_hex> <owner_secret_hex> <asset_id_hex> <bulla_blind_hex>");
        std::process::exit(1);
    }
    ...
    let asset_id = parse_hex_to_base(&args[2]).expect("Invalid asset_id hex");
...
fn run_pay_premium(args: &[String]) {
    if args.len() != 5 {
        eprintln!("Usage: pay_premium <escrow_bulla_hex> <member_secret_hex> <value_u64> <asset_id_hex> <expiry_u64>");
        std::process::exit(1);
    }
    ...
    let asset_id = parse_hex_to_base(&args[3]).expect("Invalid asset_id hex");
