...
//! Pool Stake contract test harness
...
use darkfi_pool_stake_contract::{
    model::{CreatePoolParamsV1, JoinPoolParamsV1, AllocateCoverageParamsV1},
    PoolStakeFunction,
};
...
impl TestHarness {
    /// Deploy the Pool Stake WASM contract.
    pub async fn deploy_pool_stake(
        &mut self,
        holder: &Holder,
        wasm_bincode: Vec<u8>,
        block_height: u32,
    ...
    /// Create a `PoolStake::CreatePoolV1` transaction.
    pub async fn pool_stake_create_pool(
        &mut self,
        holder: &Holder,
    ...
    /// Execute a `PoolStake::CreatePoolV1` transaction.
    pub async fn execute_pool_stake_create_pool_tx(
        &mut self,
        holder: &Holder,
        tx: Transaction,
        _params: &CreatePoolParamsV1,
        ...
        wallet.add_transaction("pool_stake::create_pool", tx, block_height).await?;
