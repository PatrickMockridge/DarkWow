...
impl TestHarness {
    ...
    /// Create a `Dice::InitializeV1` transaction.
    ///
    /// House initializes the dice game.
    pub async fn darktoshi_dice_initialize(
        &mut self,
        holder: &Holder,
        ...
        let data = vec![DiceFunction::InitializeV1 as u8];
    ...
    /// Execute a `Dice::InitializeV1` transaction.
    pub async fn execute_darktoshi_dice_initialize_tx(
        &mut self,
        holder: &Holder,
        tx: Transaction,
        ...
        wallet.add_transaction("darktoshi_dice::initialize", tx, block_height).await?;
