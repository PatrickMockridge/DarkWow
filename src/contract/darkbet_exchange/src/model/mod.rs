...
impl PlaceBackParamsV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(113); b.extend_from_slice(&self.market_id.to_repr()); b.push(self.outcome_index); b.extend_from_slice(&self.odds.to_le_bytes()); b.extend_from_slice(&self.stake.to_le_bytes()); b.extend_from_slice(&self.user_pub.to_bytes()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 113 { return Err(ContractError::IoError(format!("PlaceBackParamsV1: expected 113 bytes, got {}", data.len()))); } Ok(PlaceBackParamsV1 { market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PlaceBackParamsV1: invalid market_id".into()))?, outcome_index: data[32], odds: u32::from_le_bytes(data[33..37].try_into().unwrap()), stake: u64::from_le_bytes(data[37..45].try_into().unwrap()), user_pub: PublicKey::from_bytes(data[45..77].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PlaceBackParamsV1: invalid user_pub: {}", e)))? }) } }

impl PlaceLayParamsV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(113); b.extend_from_slice(&self.market_id.to_repr()); b.push(self.outcome_index); b.extend_from_slice(&self.odds.to_le_bytes()); b.extend_from_slice(&self.stake.to_le_bytes()); b.extend_from_slice(&self.user_pub.to_bytes()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 113 { return Err(ContractError::IoError(format!("PlaceLayParamsV1: expected 113 bytes, got {}", data.len()))); } Ok(PlaceLayParamsV1 { market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PlaceLayParamsV1: invalid market_id".into()))?, outcome_index: data[32], odds: u32::from_le_bytes(data[33..37].try_into().unwrap()), stake: u64::from_le_bytes(data[37..45].try_into().unwrap()), user_pub: PublicKey::from_bytes(data[45..77].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PlaceLayParamsV1: invalid user_pub: {}", e)))? }) } }

impl MatchOrdersParamsV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(145); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.back_order_id.to_repr()); b.extend_from_slice(&self.lay_order_id.to_repr()); b.extend_from_slice(&self.odds.to_le_bytes()); b.extend_from_slice(&self.user_pub.to_bytes()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 145 { return Err(ContractError::IoError(format!("MatchOrdersParamsV1: expected 145 bytes, got {}", data.len()))); } Ok(MatchOrdersParamsV1 { market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("MatchOrdersParamsV1: invalid market_id".into()))?, back_order_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("MatchOrdersParamsV1: invalid back_order_id".into()))?, lay_order_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[64..96].try_into().unwrap())).ok_or_else(|| ContractError::IoError("MatchOrdersParamsV1: invalid lay_order_id".into()))?, odds: u32::from_le_bytes(data[96..100].try_into().unwrap()), user_pub: PublicKey::from_bytes(data[100..132].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("MatchOrdersParamsV1: invalid user_pub: {}", e)))? }) } }
