...
//! Roulette SpinWheelV1 Client API — ZK proof for house spin authorization.
...
pub fn create_spin_wheel_proof(zkbin: &ZkBinary, pk: &ProvingKey, data: &SpinWheelCallData) -> Result<(Proof, SpinWheelPublicInputs)> {
    ...
    let w = vec![Witness::Base(Value::known(data.table_id)), Witness::Base(Value::known(data.house_secret)), Witness::Base(Value::known(data.house_pub_x)), Witness::Base(Value::known(data.house_pub_y)), Witness::Base(Value::known(data.spin_nullifier)), Witness::Base(Value::known(data.tx_commitment)), Witness::Base(Value::known(data.tx_nonce)), Witness::Base(Value::known(poseidon_hash([pallas::Base::from(3u64), data.tx_commitment, data.tx_nonce])))];
