...
//! Baccarat DrawCardsV1 Client API
//!
...
pub struct DrawCardsPublicInputs {
...
impl DrawCardsCallData {
    ...
    pub fn compute_public_inputs(&self) -> DrawCardsPublicInputs {
        DrawCardsPublicInputs {
            bet_id: self.bet_id,
...
pub fn create_draw_cards_proof(
    zkbin: &ZkBinary,
    pk: &ProvingKey,
    data: &DrawCardsCallData,
) -> Result<(Proof, DrawCardsPublicInputs)> {
    debug!(target: "contract::baccarat::client::draw_cards", "Creating DrawCardsV1 ZK proof");

    let public_inputs = DrawCardsPublicInputs {
        bet_id: data.bet_id,
