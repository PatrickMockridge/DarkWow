...
//! Baccarat settle_bet_v1 ZK proof generation
...
impl SettleBetV1PublicInputs {
    pub fn to_vec(&self) -> Vec<pallas::Base> {
        vec![self.derived_bet_id, self.tx_binding, self.tx_nonce]
    }
}
...
impl SettleBetV1CallData {
    ...
    pub fn compute_public_inputs(&self) -> SettleBetV1PublicInputs {
        ...
        SettleBetV1PublicInputs { derived_bet_id, tx_binding: pallas::Base::zero(), tx_nonce: self.tx_nonce }
    }

    pub fn to_witnesses(&self) -> Vec<Witness> {
        vec![
            // Public inputs as witnesses (prover knows them)
