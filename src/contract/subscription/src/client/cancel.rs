...
//! Subscription cancel ZK proof generation (`OBL-C106`).
//!
//! This module did not exist, and the harness proved `CancelV1` with `empty_witnesses` instead — a
//! fabricated proof carrying no instances, which the L2 verify refuses. `cancel.zk` declares six
...
/// `CancelV2`'s public inputs, in the circuit's `constrain_instance` order.
#[derive(Debug, Clone)]
...
impl CancelPublicInputs {
    pub fn to_vec(&self) -> Vec<pallas::Base> {
        // `cancel.zk`'s order: subscription_id, spent_nullifier, tx_binding, tx_nonce.
...
impl CancelCallData {
    ...
    pub fn compute_public_inputs(&self) -> CancelPublicInputs {
        CancelPublicInputs {
...
/// Create a cancel proof
pub fn create_cancel_proof(
    zkbin: &ZkBinary,
    pk: &ProvingKey,
    input: &CancelCallData,
) -> Result<(Proof, CancelPublicInputs)> {
