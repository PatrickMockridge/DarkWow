...
impl dwow_serial::Encodable for DrainConfig { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = Self::encode(self); w.write_all(&b)?; Ok(b.len()) } }
impl dwow_serial::Decodable for DrainConfig { fn decode<D: std::io::Read>(d: &mut D) -> std::io::Result<Self> { let mut buf = vec![]; d.read_to_end(&mut buf)?; Self::decode(&buf).map_err(|e| std::io::Error::other(format!("{:?}", e))) } }
