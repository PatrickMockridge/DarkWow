...
impl DrainConfig { pub fn encode(&self) -> Vec<u8> { let eq = if let Some(ref v) = self.exit_queue { v.encode() } else { vec![] }; let cb = if let Some(ref v) = self.circuit_breaker { v.encode() } else { vec![] }; let op = if let Some(ref v) = self.observation_period { v.encode() } else { vec![] }; let sp = if let Some(ref v) = self.split_proposals { v.encode() } else { vec![] }; let nl = if let Some(ref v) = self.no_loss_reserve { v.encode() } else { vec![] }; let dm = if let Some(ref v) = self.dead_mans_switch { v.encode() } else { vec![] }; let mut b = Vec::with_capacity(38+eq.len()+cb.len()+op.len()+sp.len()+nl.len()+dm.len()); b.extend_from_slice(&self.guardian_multisig_group_id.to_repr()); b.push(self.exit_queue.is_some() as u8); b.extend_from_slice(&eq); b.push(self.circuit_breaker.is_some() as u8); b.extend_from_slice(&cb); b.push(self.observation_period.is_some() as u8); b.extend_from_slice(&op); b.push(self.split_proposals.is_some() as u8); b.extend_from_slice(&sp); b.push(self.no_loss_reserve.is_some() as u8); b.extend_from_slice(&nl); b.push(self.dead_mans_switch.is_some() as u8); b.extend_from_slice(&dm); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 38 { return Err(ContractError::IoError("DrainConfig: too short".into())); } let guardian_multisig_group_id = read_base(&data[0..32])?; let mut pos = 32; let has_eq = data[pos] != 0; pos += 1; let exit_queue = if has_eq { if data.len() < pos + ExitQueueConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated exit_queue".into())); } let v = ExitQueueConfig::decode(&data[pos..pos + ExitQueueConfig::ENCODED_SIZE])?; pos += ExitQueueConfig::ENCODED_SIZE; Some(v) } else { None }; let has_cb = data[pos] != 0; pos += 1; let circuit_breaker = if has_cb { if data.len() < pos + CircuitBreakerConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated circuit_breaker".into())); } let v = CircuitBreakerConfig::decode(&data[pos..pos + CircuitBreakerConfig::ENCODED_SIZE])?; pos += CircuitBreakerConfig::ENCODED_SIZE; Some(v) } else { None }; let has_op = data[pos] != 0; pos += 1; let observation_period = if has_op { if data.len() < pos + ObservationPeriodConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated observation_period".into())); } let v = ObservationPeriodConfig::decode(&data[pos..pos + ObservationPeriodConfig::ENCODED_SIZE])?; pos += ObservationPeriodConfig::ENCODED_SIZE; Some(v) } else { None }; let has_sp = data[pos] != 0; pos += 1; let split_proposals = if has_sp { if data.len() < pos + SplitProposalsConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated split_proposals".into())); } let v = SplitProposalsConfig::decode(&data[pos..pos + SplitProposalsConfig::ENCODED_SIZE])?; pos += SplitProposalsConfig::ENCODED_SIZE; Some(v) } else { None }; let has_nl = data[pos] != 0; pos += 1; let no_loss_reserve = if has_nl { if data.len() < pos + NoLossReserveConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated no_loss_reserve".into())); } let v = NoLossReserveConfig::decode(&data[pos..pos + NoLossReserveConfig::ENCODED_SIZE])?; pos += NoLossReserveConfig::ENCODED_SIZE; Some(v) } else { None }; let has_dm = data[pos] != 0; pos += 1; let dead_mans_switch = if has_dm { if data.len() < pos + DeadMansSwitchConfig::ENCODED_SIZE { return Err(ContractError::IoError("DrainConfig: truncated dead_mans_switch".into())); } let v = DeadMansSwitchConfig::decode(&data[pos..pos + DeadMansSwitchConfig::ENCODED_SIZE])?; Some(v) } else { None }; Ok(DrainConfig { guardian_multisig_group_id, exit_queue, circuit_breaker, observation_period, split_proposals, no_loss_reserve, dead_mans_switch }) } }
...
pub struct InitializeParamsV1 {
    ...
    pub drain_config: DrainConfig,
    /// The five values `initialize.zk` instances (`OBL-C78`), derived by the client
    /// (`client::create_authority_proof`) and published by the arm. Appended after `drain_config`,
    /// whose decoding consumes a prefix rather than the whole tail, so the length check below is
    /// what fixes where the pair starts.
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl InitializeParamsV1 { pub fn encode(&self) -> Vec<u8> { let dc = self.drain_config.encode(); let mut b = Vec::with_capacity(161+dc.len()); b.extend_from_slice(&self.instance_seed); b.extend_from_slice(&self.fund_id.to_repr()); b.extend_from_slice(&self.spend_authority.to_bytes()); b.extend_from_slice(&self.dao_escrow_bulla.to_repr()); b.extend_from_slice(&dc); b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 161 { return Err(ContractError::IoError("InitializeParamsV1: too short".into())); } let instance_seed: [u8;32] = data[0..32].try_into().unwrap(); let fund_id = read_base(&data[32..64])?; let spend_authority = PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("InitializeParamsV1: invalid spend_authority: {}", e)))?; let dao_escrow_bulla = read_base(&data[96..128])?; let drain_config = DrainConfig::decode(&data[128..])?; /* `DrainConfig::decode` consumes a prefix of what it is given and does not report where it stopped, so the pair's offset is where that config *re-encodes* to — deterministic, because the encoding is canonical. `OBL-C78`. */ let dc_len = drain_config.encode().len(); let pair_at = 128usize.saturating_add(dc_len); if data.len() != pair_at.saturating_add(160) { return Err(ContractError::IoError(format!("InitializeParamsV1: expected {} bytes, got {}", pair_at.saturating_add(160), data.len()))); } Ok(InitializeParamsV1 { instance_seed, fund_id, spend_authority, dao_escrow_bulla, drain_config, authority_pub_x: read_base(&data[pair_at..pair_at+32])?, authority_pub_y: read_base(&data[pair_at+32..pair_at+64])?, authority_nullifier: read_base(&data[pair_at+64..pair_at+96])?, tx_binding: read_base(&data[pair_at+96..pair_at+128])?, tx_nonce: read_base(&data[pair_at+128..pair_at+160])? }) } }
...
pub struct ProposeParamsV1 {
    ...
    pub proof: Vec<u8>,
    /// The five values `propose.zk` instances (`OBL-C78`), same shape as `LockParamsV1`'s. Appended
    /// after the proof, so the declared proof length is what keeps the pair from being sliced out of
    /// the proof's tail.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    ...
    ///
    /// `propose_process_instruction_v1` used to look the **funds** tree up by `multisig_group_id`,
    /// a field whose name says multisig and whose key is the funds tree — so a caller had to pass a
    /// fund id in the multisig slot for a proposal to be recorded at all. A proposal belongs to a
    /// fund, so the fund is what the call names and what the arm looks up; `multisig_group_id`
    /// stays a multisig-group selector. Appended last, after the authority quintet, so every
    /// position the previous format had is unchanged.
    pub fund_id: FundId,
}

impl dwow_serial::Encodable for ProposeParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ProposeParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let mut b = Vec::with_capacity(164+self.proof.len()); b.extend_from_slice(&self.message_hash.to_repr()); b.extend_from_slice(&self.multisig_group_id.to_repr()); b.extend_from_slice(&self.prover_pubkey.to_bytes()); b.extend_from_slice(&self.vote_period_blocks.to_le_bytes()); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b.extend_from_slice(&self.fund_id.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 196 { return Err(ContractError::IoError("ProposeParamsV1: too short".into())); } let message_hash = read_base(&data[0..32])?; let multisig_group_id = read_base(&data[32..64])?; let prover_pubkey = PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("ProposeParamsV1: invalid prover_pubkey: {}", e)))?; let vote_period_blocks = u64::from_le_bytes(data[96..104].try_into().unwrap()); let proof_len = SerializedLen::from_le_bytes(data[104..108].try_into().unwrap()).to_usize(); let expected = proof_len.saturating_add(300); if data.len() != expected { return Err(ContractError::IoError(format!("ProposeParamsV1: expected {} bytes, got {}", expected, data.len()))); } let proof = data[108..108+proof_len].to_vec(); let pair_at = 108 + proof_len; Ok(ProposeParamsV1 { message_hash, multisig_group_id, prover_pubkey, vote_period_blocks, proof, authority_pub_x: read_base(&data[pair_at..pair_at+32])?, authority_pub_y: read_base(&data[pair_at+32..pair_at+64])?, authority_nullifier: read_base(&data[pair_at+64..pair_at+96])?, tx_binding: read_base(&data[pair_at+96..pair_at+128])?, tx_nonce: read_base(&data[pair_at+128..pair_at+160])?, fund_id: read_base(&data[pair_at+160..pair_at+192])? }) } }
...
#[derive(Debug, Clone,)] pub struct VoteParamsV1 { pub proposal_id: pallas::Base, pub voter_pubkey: PublicKey, pub vote: bool, pub signature: pallas::Base,
    /// The five values `vote.zk` instances (`OBL-C78`), same shape as `LockParamsV1`'s.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    pub tx_binding: pallas::Base,
    pub tx_nonce: pallas::Base,
    /// The fund the vote's proposal belongs to (`OBL-C98`), so the arm can hold the fund and check
    /// the voter's authority against it rather than recording an unattached vote.
    pub fund_id: FundId,
}
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl VoteParamsV1 { pub const ENCODED_SIZE: usize = 289; pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(289); b.extend_from_slice(&self.proposal_id.to_repr()); b.extend_from_slice(&self.voter_pubkey.to_bytes()); b.push(self.vote as u8); b.extend_from_slice(&self.signature.to_repr()); b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b.extend_from_slice(&self.fund_id.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 289 { return Err(ContractError::IoError(format!("VoteParamsV1: expected 289 bytes, got {}", data.len()))); } Ok(VoteParamsV1 { proposal_id: read_base(&data[0..32])?, voter_pubkey: PublicKey::from_bytes(data[32..64].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("VoteParamsV1: invalid voter_pubkey: {}", e)))?, vote: data[64] != 0, signature: read_base(&data[65..97])?, authority_pub_x: read_base(&data[97..129])?, authority_pub_y: read_base(&data[129..161])?, authority_nullifier: read_base(&data[161..193])?, tx_binding: read_base(&data[193..225])?, tx_nonce: read_base(&data[225..257])?, fund_id: read_base(&data[257..289])? }) } }
...
#[derive(Debug, Clone,)] pub struct ExecuteParamsV1 { pub proposal_id: pallas::Base, pub signature: pallas::Base,
    /// The five values `execute.zk` instances — `authority_pub_x`, `authority_pub_y`,
    /// `authority_nullifier`, `tx_binding`, `tx_nonce` (`OBL-C78`). The circuit derives all five from
    /// its witnesses and the metadata can only publish what the call carries, so the client computes
    /// them (`client::create_authority_proof`) and the arm publishes them. Appended after the last
    /// existing field, so every prior position is unchanged and an old-format call is refused by the
    /// length guard rather than decoded shifted.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    pub tx_binding: pallas::Base,
    ...
    /// The fund the proposal belongs to (`OBL-C98`).
    ///
    /// `execute_process_instruction_v1` used to look the **funds** tree up by `proposal_id` — a
    /// proposal id used as a fund key, which nothing writes — so no proposal could be executed at
    /// all. The fund is what the call names and what the arm looks up, which is also what lets the
    /// arm check the caller's authority against it (`OBL-C97`).
    pub fund_id: FundId,
}
...
pub struct ExitParamsV1 {
    ...
    /// — and the nonce instanced beside it (`OBL-C78`).
    ///
    /// `exit.zk` derives the binding and instances `[tx_binding, tx_nonce]` in that order; the
    /// metadata can only publish what the call carries, and the arm published two literal zeros, so
    /// `ExitV1` could not verify as built. The client computes both (`client/exit.rs`). Appended
    /// **after** the proof — which is the variable-length tail — so the decoder reads the pair from
    /// the last 64 bytes and the length check refuses an old-format call rather than mis-slicing the
    /// proof.
    pub tx_binding: pallas::Base,
    pub tx_nonce: pallas::Base,
}

impl dwow_serial::Encodable for ExitParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ExitParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let pl = SerializedLen::try_from_len(self.proof.len())?; let mut b = Vec::with_capacity(228+self.proof.len()); b.extend_from_slice(&self.fund_id.to_repr()); b.extend_from_slice(&self.member_pubkey.to_bytes()); b.extend_from_slice(&self.contribution_weight.to_le_bytes()); b.extend_from_slice(&self.current_block.to_le_bytes()); b.extend_from_slice(&self.dao_escrow_bulla.to_repr()); b.extend_from_slice(&self.dao_membership_note.to_repr()); b.extend_from_slice(&self.effective_weight.to_repr()); b.extend_from_slice(&pl.to_le_bytes()); b.extend_from_slice(&self.proof); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 228 { return Err(ContractError::IoError("ExitParamsV1: too short".into())); } let fund_id = read_base(&data[0..32])?; let member_pubkey = PublicKey::from_bytes(data[32..64].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("ExitParamsV1: invalid member_pubkey: {}", e)))?; let contribution_weight = u64::from_le_bytes(data[64..72].try_into().unwrap()); let current_block = u64::from_le_bytes(data[72..80].try_into().unwrap()); let dao_escrow_bulla = read_base(&data[80..112])?; let dao_membership_note = read_base(&data[112..144])?; let effective_weight = read_base(&data[144..176])?; let proof_len = SerializedLen::from_le_bytes(data[176..180].try_into().unwrap()).to_usize(); let expected = proof_len.saturating_add(244); if data.len() != expected { return Err(ContractError::IoError(format!("ExitParamsV1: expected {} bytes, got {}", expected, data.len()))); } let proof = data[180..180+proof_len].to_vec(); let tx_binding = read_base(&data[180+proof_len..212+proof_len])?; let tx_nonce = read_base(&data[212+proof_len..244+proof_len])?; Ok(ExitParamsV1 { fund_id, member_pubkey, contribution_weight, current_block, dao_escrow_bulla, dao_membership_note, effective_weight, proof, tx_binding, tx_nonce }) } }
...
#[derive(Debug, Clone,)] pub struct TransferParamsV1 { pub fund_id: FundId, pub amount: u64, pub recipient: PublicKey, pub signature: pallas::Base, pub exceeds_rate_limit: bool, pub vote_proposal_id: Option<pallas::Base>,
    /// The five values `transfer.zk` instances (`OBL-C78`), same shape as `LockParamsV1`'s. Appended
    /// after the optional tail, so the decoder reads them from a position the length check fixes.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    pub tx_binding: pallas::Base,
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl TransferParamsV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(170); b.extend_from_slice(&self.fund_id.to_repr()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.recipient.to_bytes()); b.extend_from_slice(&self.signature.to_repr()); b.push(self.exceeds_rate_limit as u8); b.push(self.vote_proposal_id.is_some() as u8); if let Some(v) = self.vote_proposal_id { b.extend_from_slice(&v.to_repr()); } b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 170 { return Err(ContractError::IoError("TransferParamsV1: too short".into())); } let fund_id = read_base(&data[0..32])?; let amount = u64::from_le_bytes(data[32..40].try_into().unwrap()); let recipient = PublicKey::from_bytes(data[40..72].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("TransferParamsV1: invalid recipient: {}", e)))?; let signature = read_base(&data[72..104])?; let exceeds_rate_limit = data[104] != 0; let has_vp = data[105] != 0; let vote_proposal_id = if has_vp { Some(read_base(&data[106..138])?) } else { None }; /* The pair starts where the optionals end — fixed, so the exact length is what refuses an old-format call rather than reading the pair out of the middle of one. OBL-C78. */ let pair_at = if has_vp { 138usize } else { 106usize }; if data.len() != pair_at.saturating_add(160) { return Err(ContractError::IoError(format!("TransferParamsV1: expected {} bytes, got {}", pair_at.saturating_add(160), data.len()))); } Ok(TransferParamsV1 { fund_id, amount, recipient, signature, exceeds_rate_limit, vote_proposal_id, authority_pub_x: read_base(&data[pair_at..pair_at+32])?, authority_pub_y: read_base(&data[pair_at+32..pair_at+64])?, authority_nullifier: read_base(&data[pair_at+64..pair_at+96])?, tx_binding: read_base(&data[pair_at+96..pair_at+128])?, tx_nonce: read_base(&data[pair_at+128..pair_at+160])? }) } }
...
pub struct LockParamsV1 { pub fund_id: FundId, pub duration_blocks: u64, pub signature: pallas::Base,
    /// The five values `lock.zk` instances: `authority_pub_x`, `authority_pub_y`,
    /// `authority_nullifier`, `tx_binding`, `tx_nonce` (`OBL-C78`). Derived by the client
    /// (`client::create_authority_proof`), published by the arm; appended after the last existing
    /// field so an old-format call is refused by the length guard.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    pub tx_binding: pallas::Base,
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl LockParamsV1 { pub const ENCODED_SIZE: usize = 232; pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(232); b.extend_from_slice(&self.fund_id.to_repr()); b.extend_from_slice(&self.duration_blocks.to_le_bytes()); b.extend_from_slice(&self.signature.to_repr()); b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 232 { return Err(ContractError::IoError(format!("LockParamsV1: expected 232 bytes, got {}", data.len()))); } Ok(LockParamsV1 { fund_id: read_base(&data[0..32])?, duration_blocks: u64::from_le_bytes(data[32..40].try_into().unwrap()), signature: read_base(&data[40..72])?, authority_pub_x: read_base(&data[72..104])?, authority_pub_y: read_base(&data[104..136])?, authority_nullifier: read_base(&data[136..168])?, tx_binding: read_base(&data[168..200])?, tx_nonce: read_base(&data[200..232])? }) } }
...
#[derive(Debug, Clone,)] pub struct UnlockParamsV1 { pub fund_id: FundId, pub signature: pallas::Base,
    /// The five values `unlock.zk` instances (`OBL-C78`), same shape and same reason as
    /// `LockParamsV1`'s.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
...
impl UnlockParamsV1 { pub const ENCODED_SIZE: usize = 224; pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(224); b.extend_from_slice(&self.fund_id.to_repr()); b.extend_from_slice(&self.signature.to_repr()); b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 224 { return Err(ContractError::IoError(format!("UnlockParamsV1: expected 224 bytes, got {}", data.len()))); } Ok(UnlockParamsV1 { fund_id: read_base(&data[0..32])?, signature: read_base(&data[32..64])?, authority_pub_x: read_base(&data[64..96])?, authority_pub_y: read_base(&data[96..128])?, authority_nullifier: read_base(&data[128..160])?, tx_binding: read_base(&data[160..192])?, tx_nonce: read_base(&data[192..224])? }) } }
...
pub struct UpdateConfigParamsV1 { pub fund_id: FundId, pub rate_limit: Option<RateLimit>, pub multisig_group_id: Option<pallas::Base>, pub new_spend_authority: Option<PublicKey>,
    /// The five values `update_config.zk` instances (`OBL-C78`), same shape as `LockParamsV1`'s.
    /// Appended after the optional tail; the decoder reads them from the position its cursor reaches.
    pub authority_pub_x: pallas::Base,
    pub authority_pub_y: pallas::Base,
    pub authority_nullifier: pallas::Base,
    pub tx_binding: pallas::Base,
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl UpdateConfigParamsV1 { pub fn encode(&self) -> Vec<u8> { let rl = if let Some(ref r) = self.rate_limit { r.encode() } else { vec![] }; let mut b = Vec::with_capacity(98+rl.len()); b.extend_from_slice(&self.fund_id.to_repr()); b.push(self.rate_limit.is_some() as u8); b.extend_from_slice(&rl); b.push(self.multisig_group_id.is_some() as u8); if let Some(v) = self.multisig_group_id { b.extend_from_slice(&v.to_repr()); } b.push(self.new_spend_authority.is_some() as u8); if let Some(v) = self.new_spend_authority { b.extend_from_slice(&v.to_bytes()); } b.extend_from_slice(&self.authority_pub_x.to_repr()); b.extend_from_slice(&self.authority_pub_y.to_repr()); b.extend_from_slice(&self.authority_nullifier.to_repr()); b.extend_from_slice(&self.tx_binding.to_repr()); b.extend_from_slice(&self.tx_nonce.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 98 { return Err(ContractError::IoError("UpdateConfigParamsV1: too short".into())); } let fund_id = read_base(&data[0..32])?; let has_rl = data[32] != 0; let mut pos = 33; let rate_limit = if has_rl { let r = RateLimit::decode(&data[pos..pos+24])?; pos += 24; Some(r) } else { None }; let has_mg = data[pos] != 0; pos += 1; let multisig_group_id = if has_mg { let v = read_base(&data[pos..pos+32])?; pos += 32; Some(v) } else { None }; let has_sa = data[pos] != 0; let new_spend_authority = if has_sa { let v = PublicKey::from_bytes(data[pos+1..pos+33].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("UpdateConfigParamsV1: invalid new_spend_authority: {}", e)))?; pos += 33; Some(v) } else { pos += 1; None }; /* One exact check after the optionals, covering every branch: the pair starts where the cursor stops, and an old-format call is refused rather than read from the middle. OBL-C78. */ if data.len() != pos.saturating_add(160) { return Err(ContractError::IoError(format!("UpdateConfigParamsV1: expected {} bytes, got {}", pos.saturating_add(160), data.len()))); } Ok(UpdateConfigParamsV1 { fund_id, rate_limit, multisig_group_id, new_spend_authority, authority_pub_x: read_base(&data[pos..pos+32])?, authority_pub_y: read_base(&data[pos+32..pos+64])?, authority_nullifier: read_base(&data[pos+64..pos+96])?, tx_binding: read_base(&data[pos+96..pos+128])?, tx_nonce: read_base(&data[pos+128..pos+160])? }) } }
...
impl ProtectedFund {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        ...
        buf.extend_from_slice(&SerializedLen::try_from_len(rl_enc.len())?.to_le_bytes());
        ...
        buf.extend_from_slice(&SerializedLen::try_from_len(dc_enc.len())?.to_le_bytes());
        buf.extend_from_slice(&dc_enc);
        // members: Vec<MemberWeight>
        buf.extend_from_slice(&SerializedLen::try_from_len(self.members.len())?.to_le_bytes());
        ...
        // exit_queue_state
        buf.extend_from_slice(&SerializedLen::try_from_len(self.exit_queue_state.len())?.to_le_bytes());
        ...
        // observation_pending
        buf.extend_from_slice(&SerializedLen::try_from_len(self.observation_pending.len())?.to_le_bytes());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        let mut pos: usize = 0;
        // Five prefixes, each now four bytes rather than one: 194 + 5 × 3 = 209.
        if data.len() < 209 { return Err(ContractError::IoError("ProtectedFund: data too short".into())); }
        ...
        let rl_len = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4;
        if data.len() < pos.saturating_add(rl_len) { return Err(ContractError::IoError("ProtectedFund: data too short for rate_limit".into())); }
        ...
        let dc_len = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4;
        if data.len() < pos.saturating_add(dc_len) { return Err(ContractError::IoError("ProtectedFund: data too short for drain_config".into())); }
        let drain_config = DrainConfig::decode(&data[pos..pos+dc_len])?; pos += dc_len;
        // members
        if data.len() < pos + 4 { return Err(ContractError::IoError("ProtectedFund: data too short for members count".into())); }
        let member_count = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4;
        let mut members = Vec::with_capacity(member_count);
        for _ in 0..member_count {
            if data.len() < pos.saturating_add(MemberWeight::ENCODED_SIZE) { return Err(ContractError::IoError("ProtectedFund: data too short for a member".into())); }
        ...
        // exit_queue_state
        if data.len() < pos + 4 { return Err(ContractError::IoError("ProtectedFund: data too short for exit_queue_state count".into())); }
        let eq_count = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4;
        let mut exit_queue_state = Vec::with_capacity(eq_count);
        for _ in 0..eq_count {
            if data.len() < pos.saturating_add(ExitQueueEntry::ENCODED_SIZE) { return Err(ContractError::IoError("ProtectedFund: data too short for an exit queue entry".into())); }
        ...
        // observation_pending
        if data.len() < pos + 4 { return Err(ContractError::IoError("ProtectedFund: data too short for obs count".into())); }
        let obs_count = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4;
        let mut observation_pending = Vec::with_capacity(obs_count);
        for _ in 0..obs_count {
            if data.len() < pos.saturating_add(ObservationPending::ENCODED_SIZE) { return Err(ContractError::IoError("ProtectedFund: data too short for an observation".into())); }
