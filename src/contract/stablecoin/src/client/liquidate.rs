...
pub struct LiquidatePublicInputs {
    /// Old commitment (the position this call seizes)
    pub old_commitment: pallas::Base,
    /// New commitment (the position that survives the seizure)
...
pub struct LiquidateCallData {
    ...
    /// The position this call seizes (`OBL-C83`: the circuit constrains its witnesses to
    /// reproduce this value, so it is the caller's declaration of what is being liquidated)
    pub old_commitment: pallas::Base,
...
impl LiquidateCallData {
    ...
    /// the same position, so a liquidation could never act on a position an open had created
    /// (`OBL-C83`).
...
// ============================================================================
// PromissoryNote Integration: Liquidation via spend_hook
// ============================================================================
...
// 2. Liquidator calls PromissoryNote::BurnV1 to burn stablecoin (spend_hook = stablecoin)
...
// 5. Liquidator receives seized collateral via PromissoryNote::MintV1
//
...
//
// 3. PromissoryNote calls stablecoin_contract.exec(user_data)
...
//    - Return success so PromissoryNote commits the burn
//
// 5. If stablecoin's exec() succeeds, PromissoryNote finalizes the burn
