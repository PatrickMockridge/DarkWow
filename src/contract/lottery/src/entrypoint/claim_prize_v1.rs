...
use crate::{
    ...
    LOTTERY_CONTRACT_LOTTERIES_TREE, LOTTERY_CONTRACT_PROMISSORY_NOTE_CONTRACT_ID,
...
pub fn lottery_claim_prize_process_instruction_v1(
    ...
    let params = ClaimPrizeParamsV1::decode(&self_.data[1..])?;

    // Validate children_indexes to ensure promissory_note::transfer_v1 is bundled for prize payout
    let this_call = &calls[call_idx];
    if this_call.children_indexes.len() != 1 {
        msg!(
            "[ClaimPrizeV1] Error: Expected 1 child call (promissory_note::transfer_v1), got {}",
    ...
    if child_call.data[0] != 0x04 {
        msg!(
            "[ClaimPrizeV1] Error: Expected promissory_note::transfer_v1 (0x04), got 0x{:02x}",
            child_call.data[0]
        );
    ...
    let value_blind = poseidon_hash([
        pallas::Base::from(prize),
        ticket.lottery_id,
    ]);
    validate_child_value_commit(&child_call.data, prize, value_blind)?;
