...
pub struct UnderwriteUpdateV1 {
    ...
    pub coverage_provided: u64,
    pub created_at: u64,
    /// `Underwriter::encode()` after the bond/coverage update (new or existing), as exec produced
    /// it (register OBL-C72).
    pub underwriter_bytes: Vec<u8>,
    /// `InsuranceMarket::encode()` with `coverage_sold` advanced.
...
pub struct PurchaseCoverageUpdateV1 {
    ...
    pub expires_at: u64,
    pub buyer_nullifier: pallas::Base,
    /// `Underwriter::encode()` with the premium and coverage-sold increments applied, as exec
    /// produced it (register OBL-C72).
    pub underwriter_bytes: Vec<u8>,
}
...
pub struct FileClaimUpdateV1 {
    ...
    pub created_at: u64,
    pub oracle_signature: pallas::Base,
    /// `Coverage::encode()` with `claim_id` set, as exec produced it (register OBL-C72).
...
pub struct ResolveClaimUpdateV1 {
    ...
    pub resolved_at: u64,
    pub oracle_signature: pallas::Base,
    /// `Claim::encode()` with the resolution applied, as exec produced it (register OBL-C72).
    pub claim_bytes: Vec<u8>,
    /// `Coverage::encode()` with the resolution applied.
    pub coverage_bytes: Vec<u8>,
    /// `Underwriter::encode()` when this resolution slashes the bond; absent otherwise.
    pub underwriter_bytes: Option<Vec<u8>>,
}
...
pub struct WithdrawPremiumUpdateV1 {
    ...
    /// `Underwriter::encode()` with the new balance applied, as exec produced it (OBL-C72).
    pub underwriter_bytes: Vec<u8>,
}
...
pub struct UnderwriteWithCapabilityUpdateV1 {
    ...
    pub created_at: u64,
    /// `Underwriter::encode()` after the bond/coverage update (new or existing) (register OBL-C72).
    pub underwriter_bytes: Vec<u8>,
    /// `InsuranceMarket::encode()` with `coverage_sold` advanced.
    pub market_bytes: Vec<u8>,
}
...
pub struct PurchaseCoverageWithCapabilityUpdateV1 {
    ...
    pub required_capability_id: [u8; 32],
    pub buyer_nullifier: pallas::Base,
    /// `Underwriter::encode()` with the premium and coverage-sold increments applied (OBL-C72).
    pub underwriter_bytes: Vec<u8>,
}
...
pub struct ResolveClaimWithCapabilityUpdateV1 {
    ...
    pub resolved_at: u64,
    pub oracle_signature: pallas::Base,
    /// `Claim::encode()` with the resolution applied (OBL-C72).
    pub claim_bytes: Vec<u8>,
    /// `Coverage::encode()` with the resolution applied.
    pub coverage_bytes: Vec<u8>,
    /// `Underwriter::encode()` when this resolution slashes the bond; absent otherwise.
...
pub struct DeactivateUnderwriterUpdateV1 {
    pub underwriter_id: UnderwriterId,
    /// `Underwriter::encode()` with `active = false`, as exec produced it (register OBL-C72).
    pub underwriter_bytes: Vec<u8>,
}
...
pub struct CloseMarketUpdateV1 {
    pub market_id: MarketId,
    /// `InsuranceMarket::encode()` with `active = false`, as exec produced it. Carried so apply
    /// re-stores it rather than reading it back — the read triad is denied in `Update` (register
    /// OBL-C72).
    pub market_bytes: Vec<u8>,
}
...
pub struct RetireRiskTypeUpdateV1 {
    pub risk_type_id: RiskTypeId,
    /// `RiskType::encode()` with `active = false`, as exec produced it (register OBL-C72).
    pub risk_type_bytes: Vec<u8>,
}

// --- Rho-calculus encode/decode for parameter types ---
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl RegisterRiskTypeParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.description.len())?; let mut b = Vec::with_capacity(73+self.description.len()); b.push(self.category as u8); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.description); b.extend_from_slice(&self.base_premium_rate.to_le_bytes()); b.extend_from_slice(&self.min_bond_rate.to_le_bytes()); b.extend_from_slice(&self.oracle_pubkey.to_bytes()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 45 { return Err(ContractError::IoError("RegisterRiskTypeParamsV1: too short".into())); } let category = RiskCategory::decode(&data[0..1])?; let desc_len = SerializedLen::from_le_bytes(data[1..5].try_into().unwrap()).to_usize(); let expected = desc_len.saturating_add(45); if data.len() != expected { return Err(ContractError::IoError(format!("RegisterRiskTypeParamsV1: expected {} got {}", expected, data.len()))); } Ok(RegisterRiskTypeParamsV1 { category, description: data[5..5+desc_len].to_vec(), base_premium_rate: u32::from_le_bytes(data[5+desc_len..9+desc_len].try_into().unwrap()), min_bond_rate: u32::from_le_bytes(data[9+desc_len..13+desc_len].try_into().unwrap()), oracle_pubkey: PublicKey::from_bytes(data[13+desc_len..45+desc_len].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("RegisterRiskTypeParamsV1: invalid oracle_pubkey: {}", e)))? }) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageParamsV1 { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(168); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.coverage_amount.to_le_bytes()); b.extend_from_slice(&self.value_commit.to_bytes()); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() != 168 { return Err(ContractError::IoError(format!("PurchaseCoverageParamsV1: expected 168 got {}", data.len()))); } Ok(PurchaseCoverageParamsV1 { market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageParamsV1: invalid market_id".into()))?, underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageParamsV1: invalid underwriter_id".into()))?, buyer: PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageParamsV1: invalid buyer: {}", e)))?, coverage_amount: u64::from_le_bytes(data[96..104].try_into().unwrap()), value_commit: Option::<pallas::Point>::from(pallas::Point::from_bytes(data[104..136].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageParamsV1: invalid value_commit".into()))?, buyer_nullifier: Option::<pallas::Base>::from(pallas::Base::from_repr(data[136..168].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageParamsV1: invalid buyer_nullifier".into()))? }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl FileClaimParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.evidence.len())?; let mut b = Vec::with_capacity(140+self.evidence.len()); b.extend_from_slice(&self.coverage_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.evidence); b.extend_from_slice(&self.oracle_signature.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 140 { return Err(ContractError::IoError("FileClaimParamsV1: too short".into())); } let coverage_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimParamsV1: invalid coverage_id".into()))?; let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimParamsV1: invalid market_id".into()))?; let buyer = PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("FileClaimParamsV1: invalid buyer: {}", e)))?; let amount = u64::from_le_bytes(data[96..104].try_into().unwrap()); let ev_len = SerializedLen::from_le_bytes(data[104..108].try_into().unwrap()).to_usize(); let ev_end = ev_len.saturating_add(108); if data.len() < ev_end+32 { return Err(ContractError::IoError("FileClaimParamsV1: truncated".into())); } let evidence = data[108..ev_end].to_vec(); let oracle_signature = Option::<pallas::Base>::from(pallas::Base::from_repr(data[ev_end..ev_end+32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimParamsV1: invalid oracle_signature".into()))?; Ok(FileClaimParamsV1 { coverage_id, market_id, buyer, amount, evidence, oracle_signature }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl UnderwriteWithCapabilityParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.capability_proof.len())?; let mut b = Vec::with_capacity(84+self.capability_proof.len()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.bond_amount.to_le_bytes()); b.extend_from_slice(&self.coverage_limit.to_le_bytes()); b.extend_from_slice(&self.underwriter.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.capability_proof); b.extend_from_slice(&self.capability_secret); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 116 { return Err(ContractError::IoError("UnderwriteWithCapabilityParamsV1: too short".into())); } let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("UnderwriteWithCapabilityParamsV1: invalid market_id".into()))?; let bond_amount = u64::from_le_bytes(data[32..40].try_into().unwrap()); let coverage_limit = u64::from_le_bytes(data[40..48].try_into().unwrap()); let underwriter = PublicKey::from_bytes(data[48..80].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("UnderwriteWithCapabilityParamsV1: invalid underwriter: {}", e)))?; let cp_len = SerializedLen::from_le_bytes(data[80..84].try_into().unwrap()).to_usize(); let expected = cp_len.saturating_add(116); if data.len() != expected { return Err(ContractError::IoError(format!("UnderwriteWithCapabilityParamsV1: expected {} got {}", expected, data.len()))); } let capability_proof = data[84..84+cp_len].to_vec(); let capability_secret: [u8;32] = data[84+cp_len..84+cp_len+32].try_into().unwrap(); Ok(UnderwriteWithCapabilityParamsV1 { market_id, bond_amount, coverage_limit, underwriter, capability_proof, capability_secret }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageWithCapabilityParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.capability_proof.len())?; let mut b = Vec::with_capacity(172+self.capability_proof.len()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.coverage_amount.to_le_bytes()); b.extend_from_slice(&self.value_commit.to_bytes()); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.capability_proof); b.extend_from_slice(&self.capability_secret); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 204 { return Err(ContractError::IoError("PurchaseCoverageWithCapabilityParamsV1: too short".into())); } let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityParamsV1: invalid market_id".into()))?; let underwriter_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityParamsV1: invalid underwriter_id".into()))?; let buyer = PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageWithCapabilityParamsV1: invalid buyer: {}", e)))?; let coverage_amount = u64::from_le_bytes(data[96..104].try_into().unwrap()); let value_commit = Option::<pallas::Point>::from(pallas::Point::from_bytes(data[104..136].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityParamsV1: invalid value_commit".into()))?; let buyer_nullifier = Option::<pallas::Base>::from(pallas::Base::from_repr(data[136..168].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityParamsV1: invalid buyer_nullifier".into()))?; let cp_len = SerializedLen::from_le_bytes(data[168..172].try_into().unwrap()).to_usize(); let expected = cp_len.saturating_add(204); if data.len() != expected { return Err(ContractError::IoError(format!("PurchaseCoverageWithCapabilityParamsV1: expected {} got {}", expected, data.len()))); } let capability_proof = data[172..172+cp_len].to_vec(); let capability_secret: [u8;32] = data[172+cp_len..172+cp_len+32].try_into().unwrap(); Ok(PurchaseCoverageWithCapabilityParamsV1 { market_id, underwriter_id, buyer, coverage_amount, value_commit, buyer_nullifier, capability_proof, capability_secret }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ResolveClaimWithCapabilityParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let an = SerializedLen::try_from_len(self.attestation.len())?; let cn = SerializedLen::try_from_len(self.capability_proof.len())?; let mut b = Vec::with_capacity(113+self.attestation.len()+self.capability_proof.len()); b.extend_from_slice(&self.claim_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.push(self.is_valid as u8); b.extend_from_slice(&self.payout_amount.to_le_bytes()); b.extend_from_slice(&an.to_le_bytes()); b.extend_from_slice(&self.attestation); b.extend_from_slice(&self.oracle_signature.to_repr()); b.extend_from_slice(&cn.to_le_bytes()); b.extend_from_slice(&self.capability_proof); b.extend_from_slice(&self.capability_secret); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 145 { return Err(ContractError::IoError("ResolveClaimWithCapabilityParamsV1: too short".into())); } let claim_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityParamsV1: invalid claim_id".into()))?; let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityParamsV1: invalid market_id".into()))?; let is_valid = data[64] != 0; let payout_amount = u64::from_le_bytes(data[65..73].try_into().unwrap()); let att_len = SerializedLen::from_le_bytes(data[73..77].try_into().unwrap()).to_usize(); let att_end = att_len.saturating_add(77); if data.len() < att_end.saturating_add(36) { return Err(ContractError::IoError("ResolveClaimWithCapabilityParamsV1: truncated".into())); } let attestation = data[77..att_end].to_vec(); let oracle_signature = Option::<pallas::Base>::from(pallas::Base::from_repr(data[att_end..att_end+32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityParamsV1: invalid oracle_signature".into()))?; let cp_len = SerializedLen::from_le_bytes(data[att_end+32..att_end+36].try_into().unwrap()).to_usize(); let expected = att_end.saturating_add(36).saturating_add(cp_len).saturating_add(32); if data.len() != expected { return Err(ContractError::IoError(format!("ResolveClaimWithCapabilityParamsV1: expected {} got {}", expected, data.len()))); } let capability_proof = data[att_end+36..att_end+36+cp_len].to_vec(); let capability_secret: [u8;32] = data[att_end+36+cp_len..att_end+36+cp_len+32].try_into().unwrap(); Ok(ResolveClaimWithCapabilityParamsV1 { claim_id, market_id, is_valid, payout_amount, attestation, oracle_signature, capability_proof, capability_secret }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ResolveClaimParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.attestation.len())?; let mut b = Vec::with_capacity(109+self.attestation.len()); b.extend_from_slice(&self.claim_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.push(self.is_valid as u8); b.extend_from_slice(&self.payout_amount.to_le_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.attestation); b.extend_from_slice(&self.oracle_signature.to_repr()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 109 { return Err(ContractError::IoError("ResolveClaimParamsV1: too short".into())); } let claim_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimParamsV1: invalid claim_id".into()))?; let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimParamsV1: invalid market_id".into()))?; let is_valid = data[64] != 0; let payout_amount = u64::from_le_bytes(data[65..73].try_into().unwrap()); let att_len = SerializedLen::from_le_bytes(data[73..77].try_into().unwrap()).to_usize(); let expected = att_len.saturating_add(109); if data.len() != expected { return Err(ContractError::IoError(format!("ResolveClaimParamsV1: expected {} got {}", expected, data.len()))); } let attestation = data[77..77+att_len].to_vec(); let oracle_signature = Option::<pallas::Base>::from(pallas::Base::from_repr(data[77+att_len..109+att_len].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimParamsV1: invalid oracle_signature".into()))?; Ok(ResolveClaimParamsV1 { claim_id, market_id, is_valid, payout_amount, attestation, oracle_signature }) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageWithDAGParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.dag_proof.len())?; let mut b = Vec::with_capacity(204+self.dag_proof.len()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.coverage_amount.to_le_bytes()); b.extend_from_slice(&self.value_commit.to_bytes()); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.dag_proof); b.extend_from_slice(&self.dag_path_index.to_le_bytes()); b.extend_from_slice(&self.required_dag_id); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 204 { return Err(ContractError::IoError("PurchaseCoverageWithDAGParamsV1: too short".into())); } let market_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGParamsV1: invalid market_id".into()))?; let underwriter_id = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGParamsV1: invalid underwriter_id".into()))?; let buyer = PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageWithDAGParamsV1: invalid buyer: {}", e)))?; let coverage_amount = u64::from_le_bytes(data[96..104].try_into().unwrap()); let value_commit = Option::<pallas::Point>::from(pallas::Point::from_bytes(data[104..136].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGParamsV1: invalid value_commit".into()))?; let buyer_nullifier = Option::<pallas::Base>::from(pallas::Base::from_repr(data[136..168].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGParamsV1: invalid buyer_nullifier".into()))?; let dp_len = SerializedLen::from_le_bytes(data[168..172].try_into().unwrap()).to_usize(); let dp_end = dp_len.saturating_add(172); if data.len() < dp_end.saturating_add(36) { return Err(ContractError::IoError("PurchaseCoverageWithDAGParamsV1: truncated".into())); } let dag_proof = data[172..dp_end].to_vec(); let dag_path_index = u32::from_le_bytes(data[dp_end..dp_end+4].try_into().unwrap()); let required_dag_id: [u8;32] = data[dp_end+4..dp_end+36].try_into().unwrap(); Ok(PurchaseCoverageWithDAGParamsV1 { market_id, underwriter_id, buyer, coverage_amount, value_commit, buyer_nullifier, dag_proof, dag_path_index, required_dag_id }) } }
...
impl RiskType {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let mut b = Vec::with_capacity(87 + self.description.len());
        b.push(self.version);
        b.extend_from_slice(&self.id.to_repr());
        b.push(self.category as u8);
        b.extend_from_slice(&SerializedLen::try_from_len(self.description.len())?.to_le_bytes());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let desc_len = SerializedLen::from_le_bytes(data[34..38].try_into().unwrap()).to_usize();
        let expected = desc_len.saturating_add(87);
        if data.len() != expected { return Err(ContractError::IoError(format!("RiskType: expected {} bytes, got {}", expected, data.len()))); }
        let description = data[38..38+desc_len].to_vec();
        let base_premium_rate = u32::from_le_bytes(data[38+desc_len..42+desc_len].try_into().unwrap());
        let min_bond_rate = u32::from_le_bytes(data[42+desc_len..46+desc_len].try_into().unwrap());
        let oracle_pubkey = PublicKey::from_bytes(data[46+desc_len..78+desc_len].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("RiskType: invalid oracle_pubkey: {}", e)))?;
        let active = data[78+desc_len] != 0;
        let created_at = u64::from_le_bytes(data[79+desc_len..87+desc_len].try_into().unwrap());
...
impl Claim {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let cap = 162 + self.evidence.len() + self.attestation.len();
        let mut b = Vec::with_capacity(cap);
        b.push(self.version);
        b.extend_from_slice(&self.id.to_repr());
        ...
        b.push(self.state as u8);
        b.extend_from_slice(&SerializedLen::try_from_len(self.evidence.len())?.to_le_bytes());
        b.extend_from_slice(&self.evidence);
        b.extend_from_slice(&SerializedLen::try_from_len(self.attestation.len())?.to_le_bytes());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        // version(1)+id(32)+coverage_id(32)+market_id(32)+amount(8)+payout(8)+state(1)
        // + evidence length(4) + attestation length(4) + oracle_signature(32) + resolved_at(9) = 162
        if data.len() < 162 { return Err(ContractError::IoError(format!("Claim:expected at least 156 bytes, got {}", data.len()))); }
        let version = data[0];
        ...
        let ev_len = SerializedLen::from_le_bytes(data[114..118].try_into().unwrap()).to_usize();
        let ev_end = ev_len.saturating_add(118);
        if data.len() < ev_end + 4 { return Err(ContractError::IoError("Claim: data too short for evidence".into())); }
        let evidence = data[118..ev_end].to_vec();
        let at_len = SerializedLen::from_le_bytes(data[ev_end..ev_end+4].try_into().unwrap()).to_usize();
        let at_end = at_len.saturating_add(ev_end).saturating_add(4);
        if data.len() != at_end + 40 { return Err(ContractError::IoError(format!("Claim: expected {} bytes, got {}", at_end + 40, data.len()))); }
        let attestation = data[ev_end+4..at_end].to_vec();
...
impl EndowmentPool {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let mut b = Vec::with_capacity(101 + self.returns_history.len() * 8);
        b.push(self.version);
        b.extend_from_slice(&self.id.to_repr());
        b.extend_from_slice(&self.market_id.to_repr());
        ...
        b.extend_from_slice(&SerializedLen::try_from_len(self.returns_history.len())?.to_le_bytes());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let rh_count = SerializedLen::from_le_bytes(data[89..93].try_into().unwrap()).to_usize();
        let expected = rh_count.saturating_mul(8).saturating_add(101);
        ...
        for i in 0..rh_count { returns_history.push(u64::from_le_bytes(data[93+i*8..93+(i+1)*8].try_into().unwrap())); }
        let created_at = u64::from_le_bytes(data[93+rh_count*8..101+rh_count*8].try_into().unwrap());
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl UnderwriteUpdateV1 { pub const FIXED: usize = 128; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let un = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mn = SerializedLen::try_from_len(self.market_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len() + self.market_bytes.len()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.owner.to_bytes()); b.extend_from_slice(&self.bond_amount.to_le_bytes()); b.extend_from_slice(&self.coverage_provided.to_le_bytes()); b.extend_from_slice(&self.created_at.to_le_bytes()); b.extend_from_slice(&un.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); b.extend_from_slice(&mn.to_le_bytes()); b.extend_from_slice(&self.market_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("UnderwriteUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let un = SerializedLen::from_le_bytes(data[120..124].try_into().unwrap()).to_usize(); let mpos = 124usize.saturating_add(un); if data.len() < mpos + 4 { return Err(ContractError::IoError("UnderwriteUpdateV1: truncated".into())); } let mn = SerializedLen::from_le_bytes(data[mpos..mpos+4].try_into().unwrap()).to_usize(); if data.len() != mpos + 4 + mn { return Err(ContractError::IoError(format!("UnderwriteUpdateV1: {} underwriter and {} market bytes do not fit {} total", un, mn, data.len()))); } Ok(UnderwriteUpdateV1 { underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("UnderwriteUpdateV1: invalid underwriter_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("UnderwriteUpdateV1: invalid market_id".into()))?, owner: PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("UnderwriteUpdateV1: invalid owner: {}", e)))?, bond_amount: u64::from_le_bytes(data[96..104].try_into().unwrap()), coverage_provided: u64::from_le_bytes(data[104..112].try_into().unwrap()), created_at: u64::from_le_bytes(data[112..120].try_into().unwrap()), underwriter_bytes: data[124..mpos].to_vec(), market_bytes: data[mpos+4..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageUpdateV1 { pub const FIXED: usize = 196; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len()); b.extend_from_slice(&self.coverage_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.premium_paid.to_le_bytes()); b.extend_from_slice(&self.starts_at.to_le_bytes()); b.extend_from_slice(&self.expires_at.to_le_bytes()); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("PurchaseCoverageUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[192..196].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("PurchaseCoverageUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(PurchaseCoverageUpdateV1 { coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageUpdateV1: invalid coverage_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageUpdateV1: invalid market_id".into()))?, underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[64..96].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageUpdateV1: invalid underwriter_id".into()))?, buyer: PublicKey::from_bytes(data[96..128].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageUpdateV1: invalid buyer: {}", e)))?, amount: u64::from_le_bytes(data[128..136].try_into().unwrap()), premium_paid: u64::from_le_bytes(data[136..144].try_into().unwrap()), starts_at: u64::from_le_bytes(data[144..152].try_into().unwrap()), expires_at: u64::from_le_bytes(data[152..160].try_into().unwrap()), buyer_nullifier: Option::<pallas::Base>::from(pallas::Base::from_repr(data[160..192].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageUpdateV1: invalid buyer_nullifier".into()))?, underwriter_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl WithdrawPremiumUpdateV1 { pub const FIXED: usize = 52; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.remaining_balance.to_le_bytes()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("WithdrawPremiumUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[48..52].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("WithdrawPremiumUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(WithdrawPremiumUpdateV1 { underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("WithdrawPremiumUpdateV1: invalid underwriter_id".into()))?, amount: u64::from_le_bytes(data[32..40].try_into().unwrap()), remaining_balance: u64::from_le_bytes(data[40..48].try_into().unwrap()), underwriter_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ResolveClaimUpdateV1 { pub const FIXED: usize = 133; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let cn = SerializedLen::try_from_len(self.claim_bytes.len())?; let vn = SerializedLen::try_from_len(self.coverage_bytes.len())?; let un = SerializedLen::try_from_len(self.underwriter_bytes.as_ref().map_or(0, |v| v.len()))?; let mut b = Vec::with_capacity(Self::FIXED + self.claim_bytes.len() + self.coverage_bytes.len() + un.to_usize()); b.extend_from_slice(&self.claim_id.to_repr()); b.extend_from_slice(&self.coverage_id.to_repr()); b.push(self.is_valid as u8); b.extend_from_slice(&self.payout_amount.to_le_bytes()); b.extend_from_slice(&self.slash_amount.to_le_bytes()); b.extend_from_slice(&self.resolved_at.to_le_bytes()); b.extend_from_slice(&self.oracle_signature.to_repr()); b.extend_from_slice(&cn.to_le_bytes()); b.extend_from_slice(&self.claim_bytes); b.extend_from_slice(&vn.to_le_bytes()); b.extend_from_slice(&self.coverage_bytes); b.push(self.underwriter_bytes.is_some() as u8); if let Some(u) = &self.underwriter_bytes { b.extend_from_slice(&un.to_le_bytes()); b.extend_from_slice(u); } Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("ResolveClaimUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let cn = SerializedLen::from_le_bytes(data[121..125].try_into().unwrap()).to_usize(); let cpos = 125usize.saturating_add(cn); if data.len() < cpos + 4 { return Err(ContractError::IoError("ResolveClaimUpdateV1: truncated".into())); } let vn = SerializedLen::from_le_bytes(data[cpos..cpos+4].try_into().unwrap()).to_usize(); let vpos = cpos.saturating_add(4).saturating_add(vn); if data.len() < vpos + 1 { return Err(ContractError::IoError("ResolveClaimUpdateV1: truncated".into())); } let has_u = data[vpos] != 0; let underwriter_bytes = if has_u { if data.len() < vpos + 5 { return Err(ContractError::IoError("ResolveClaimUpdateV1: truncated underwriter header".into())); } let un = SerializedLen::from_le_bytes(data[vpos+1..vpos+5].try_into().unwrap()).to_usize(); if data.len() != vpos + 5 + un { return Err(ContractError::IoError(format!("ResolveClaimUpdateV1: {} claim, {} coverage and {} underwriter bytes do not fit {} total", cn, vn, un, data.len()))); } Some(data[vpos+5..].to_vec()) } else { if data.len() != vpos + 1 { return Err(ContractError::IoError("ResolveClaimUpdateV1: trailing bytes after an absent underwriter".into())); } None }; Ok(ResolveClaimUpdateV1 { claim_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimUpdateV1: invalid claim_id".into()))?, coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimUpdateV1: invalid coverage_id".into()))?, is_valid: data[64] != 0, payout_amount: u64::from_le_bytes(data[65..73].try_into().unwrap()), slash_amount: u64::from_le_bytes(data[73..81].try_into().unwrap()), resolved_at: u64::from_le_bytes(data[81..89].try_into().unwrap()), oracle_signature: Option::<pallas::Base>::from(pallas::Base::from_repr(data[89..121].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimUpdateV1: invalid oracle_signature".into()))?, claim_bytes: data[125..cpos].to_vec(), coverage_bytes: data[cpos+4..vpos].to_vec(), underwriter_bytes }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl FileClaimUpdateV1 { pub const FIXED: usize = 149; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.coverage_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.coverage_bytes.len()); b.extend_from_slice(&self.claim_id.to_repr()); b.extend_from_slice(&self.coverage_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.amount.to_le_bytes()); b.push(self.state as u8); b.extend_from_slice(&self.created_at.to_le_bytes()); b.extend_from_slice(&self.oracle_signature.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.coverage_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("FileClaimUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[145..149].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("FileClaimUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(FileClaimUpdateV1 { claim_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimUpdateV1: invalid claim_id".into()))?, coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimUpdateV1: invalid coverage_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[64..96].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimUpdateV1: invalid market_id".into()))?, amount: u64::from_le_bytes(data[96..104].try_into().unwrap()), state: ClaimState::try_from(data[104])?, created_at: u64::from_le_bytes(data[105..113].try_into().unwrap()), oracle_signature: Option::<pallas::Base>::from(pallas::Base::from_repr(data[113..145].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FileClaimUpdateV1: invalid oracle_signature".into()))?, coverage_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl DeactivateUnderwriterUpdateV1 { pub const FIXED: usize = 36; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("DeactivateUnderwriterUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("DeactivateUnderwriterUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(DeactivateUnderwriterUpdateV1 { underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("DeactivateUnderwriterUpdateV1: invalid underwriter_id".into()))?, underwriter_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl CloseMarketUpdateV1 { pub const FIXED: usize = 36; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.market_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.market_bytes.len()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.market_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("CloseMarketUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("CloseMarketUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(CloseMarketUpdateV1 { market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("CloseMarketUpdateV1: invalid market_id".into()))?, market_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl RetireRiskTypeUpdateV1 { pub const FIXED: usize = 36; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.risk_type_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.risk_type_bytes.len()); b.extend_from_slice(&self.risk_type_id.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.risk_type_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("RetireRiskTypeUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[32..36].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("RetireRiskTypeUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(RetireRiskTypeUpdateV1 { risk_type_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("RetireRiskTypeUpdateV1: invalid risk_type_id".into()))?, risk_type_bytes: data[Self::FIXED..].to_vec() }) } }
...
impl RegisterRiskTypeUpdateV1 {
    pub fn encode(&self) -> Result<Vec<u8>, ContractError> {
        let n = SerializedLen::try_from_len(self.description.len())?;
        let mut b = Vec::with_capacity(85 + self.description.len());
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        if data.len() < 85 { return Err(ContractError::IoError(format!("RegisterRiskTypeUpdateV1: expected at least 85 bytes, got {}", data.len()))); }
        ...
        let desc_len = SerializedLen::from_le_bytes(data[33..37].try_into().unwrap()).to_usize();
        let expected = desc_len.saturating_add(85);
        if data.len() != expected { return Err(ContractError::IoError(format!("RegisterRiskTypeUpdateV1: expected {} bytes, got {}", expected + desc_len, data.len()))); }
        let description = data[37..37+desc_len].to_vec();
        let base_premium_rate = u32::from_le_bytes(data[37+desc_len..41+desc_len].try_into().unwrap());
        let min_bond_rate = u32::from_le_bytes(data[41+desc_len..45+desc_len].try_into().unwrap());
        let oracle_pubkey = PublicKey::from_bytes(data[45+desc_len..77+desc_len].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("RegisterRiskTypeUpdateV1: invalid oracle_pubkey: {}", e)))?;
        let created_at = u64::from_le_bytes(data[77+desc_len..85+desc_len].try_into().unwrap());
        Ok(RegisterRiskTypeUpdateV1 { risk_type_id, category, description, base_premium_rate, min_bond_rate, oracle_pubkey, created_at })
    }
}

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl UnderwriteWithCapabilityUpdateV1 { pub const FIXED: usize = 160; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let un = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mn = SerializedLen::try_from_len(self.market_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len() + self.market_bytes.len()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.owner.to_bytes()); b.extend_from_slice(&self.bond_amount.to_le_bytes()); b.extend_from_slice(&self.coverage_provided.to_le_bytes()); b.extend_from_slice(&self.required_capability_id); b.extend_from_slice(&self.created_at.to_le_bytes()); b.extend_from_slice(&un.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); b.extend_from_slice(&mn.to_le_bytes()); b.extend_from_slice(&self.market_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("UnderwriteWithCapabilityUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let un = SerializedLen::from_le_bytes(data[152..156].try_into().unwrap()).to_usize(); let mpos = 156usize.saturating_add(un); if data.len() < mpos + 4 { return Err(ContractError::IoError("UnderwriteWithCapabilityUpdateV1: truncated".into())); } let mn = SerializedLen::from_le_bytes(data[mpos..mpos+4].try_into().unwrap()).to_usize(); if data.len() != mpos + 4 + mn { return Err(ContractError::IoError(format!("UnderwriteWithCapabilityUpdateV1: {} underwriter and {} market bytes do not fit {} total", un, mn, data.len()))); } Ok(UnderwriteWithCapabilityUpdateV1 { underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("UnderwriteWithCapabilityUpdateV1: invalid underwriter_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("UnderwriteWithCapabilityUpdateV1: invalid market_id".into()))?, owner: PublicKey::from_bytes(data[64..96].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("UnderwriteWithCapabilityUpdateV1: invalid owner: {}", e)))?, bond_amount: u64::from_le_bytes(data[96..104].try_into().unwrap()), coverage_provided: u64::from_le_bytes(data[104..112].try_into().unwrap()), required_capability_id: data[112..144].try_into().unwrap(), created_at: u64::from_le_bytes(data[144..152].try_into().unwrap()), underwriter_bytes: data[156..mpos].to_vec(), market_bytes: data[mpos+4..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageWithCapabilityUpdateV1 { pub const FIXED: usize = 228; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len()); b.extend_from_slice(&self.coverage_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.premium_paid.to_le_bytes()); b.extend_from_slice(&self.starts_at.to_le_bytes()); b.extend_from_slice(&self.expires_at.to_le_bytes()); b.extend_from_slice(&self.required_capability_id); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("PurchaseCoverageWithCapabilityUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[224..228].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("PurchaseCoverageWithCapabilityUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(PurchaseCoverageWithCapabilityUpdateV1 { coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityUpdateV1: invalid coverage_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityUpdateV1: invalid market_id".into()))?, underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[64..96].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityUpdateV1: invalid underwriter_id".into()))?, buyer: PublicKey::from_bytes(data[96..128].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageWithCapabilityUpdateV1: invalid buyer: {}", e)))?, amount: u64::from_le_bytes(data[128..136].try_into().unwrap()), premium_paid: u64::from_le_bytes(data[136..144].try_into().unwrap()), starts_at: u64::from_le_bytes(data[144..152].try_into().unwrap()), expires_at: u64::from_le_bytes(data[152..160].try_into().unwrap()), required_capability_id: data[160..192].try_into().unwrap(), buyer_nullifier: Option::<pallas::Base>::from(pallas::Base::from_repr(data[192..224].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithCapabilityUpdateV1: invalid buyer_nullifier".into()))?, underwriter_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl PurchaseCoverageWithDAGUpdateV1 { pub const FIXED: usize = 232; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.underwriter_bytes.len())?; let mut b = Vec::with_capacity(Self::FIXED + self.underwriter_bytes.len()); b.extend_from_slice(&self.coverage_id.to_repr()); b.extend_from_slice(&self.market_id.to_repr()); b.extend_from_slice(&self.underwriter_id.to_repr()); b.extend_from_slice(&self.buyer.to_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.premium_paid.to_le_bytes()); b.extend_from_slice(&self.starts_at.to_le_bytes()); b.extend_from_slice(&self.expires_at.to_le_bytes()); b.extend_from_slice(&self.required_dag_id); b.extend_from_slice(&self.dag_path_satisfied.to_le_bytes()); b.extend_from_slice(&self.buyer_nullifier.to_repr()); b.extend_from_slice(&n.to_le_bytes()); b.extend_from_slice(&self.underwriter_bytes); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("PurchaseCoverageWithDAGUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let n = SerializedLen::from_le_bytes(data[228..232].try_into().unwrap()).to_usize(); if data.len() != Self::FIXED.saturating_add(n) { return Err(ContractError::IoError(format!("PurchaseCoverageWithDAGUpdateV1: {} record bytes do not fit {} total", n, data.len()))); } Ok(PurchaseCoverageWithDAGUpdateV1 { coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGUpdateV1: invalid coverage_id".into()))?, market_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGUpdateV1: invalid market_id".into()))?, underwriter_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[64..96].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGUpdateV1: invalid underwriter_id".into()))?, buyer: PublicKey::from_bytes(data[96..128].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("PurchaseCoverageWithDAGUpdateV1: invalid buyer: {}", e)))?, amount: u64::from_le_bytes(data[128..136].try_into().unwrap()), premium_paid: u64::from_le_bytes(data[136..144].try_into().unwrap()), starts_at: u64::from_le_bytes(data[144..152].try_into().unwrap()), expires_at: u64::from_le_bytes(data[152..160].try_into().unwrap()), required_dag_id: data[160..192].try_into().unwrap(), dag_path_satisfied: u32::from_le_bytes(data[192..196].try_into().unwrap()), buyer_nullifier: Option::<pallas::Base>::from(pallas::Base::from_repr(data[196..228].try_into().unwrap())).ok_or_else(|| ContractError::IoError("PurchaseCoverageWithDAGUpdateV1: invalid buyer_nullifier".into()))?, underwriter_bytes: data[Self::FIXED..].to_vec() }) } }

#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ResolveClaimWithCapabilityUpdateV1 { pub const FIXED: usize = 133; pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let cn = SerializedLen::try_from_len(self.claim_bytes.len())?; let vn = SerializedLen::try_from_len(self.coverage_bytes.len())?; let un = SerializedLen::try_from_len(self.underwriter_bytes.as_ref().map_or(0, |v| v.len()))?; let mut b = Vec::with_capacity(Self::FIXED + self.claim_bytes.len() + self.coverage_bytes.len() + un.to_usize()); b.extend_from_slice(&self.claim_id.to_repr()); b.extend_from_slice(&self.coverage_id.to_repr()); b.push(self.is_valid as u8); b.extend_from_slice(&self.payout_amount.to_le_bytes()); b.extend_from_slice(&self.slash_amount.to_le_bytes()); b.extend_from_slice(&self.resolved_at.to_le_bytes()); b.extend_from_slice(&self.oracle_signature.to_repr()); b.extend_from_slice(&cn.to_le_bytes()); b.extend_from_slice(&self.claim_bytes); b.extend_from_slice(&vn.to_le_bytes()); b.extend_from_slice(&self.coverage_bytes); b.push(self.underwriter_bytes.is_some() as u8); if let Some(u) = &self.underwriter_bytes { b.extend_from_slice(&un.to_le_bytes()); b.extend_from_slice(u); } Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < Self::FIXED { return Err(ContractError::IoError(format!("ResolveClaimWithCapabilityUpdateV1: expected at least {} bytes, got {}", Self::FIXED, data.len()))); } let cn = SerializedLen::from_le_bytes(data[121..125].try_into().unwrap()).to_usize(); let cpos = 125usize.saturating_add(cn); if data.len() < cpos + 4 { return Err(ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: truncated".into())); } let vn = SerializedLen::from_le_bytes(data[cpos..cpos+4].try_into().unwrap()).to_usize(); let vpos = cpos.saturating_add(4).saturating_add(vn); if data.len() < vpos + 1 { return Err(ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: truncated".into())); } let has_u = data[vpos] != 0; let underwriter_bytes = if has_u { if data.len() < vpos + 5 { return Err(ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: truncated underwriter header".into())); } let un = SerializedLen::from_le_bytes(data[vpos+1..vpos+5].try_into().unwrap()).to_usize(); if data.len() != vpos + 5 + un { return Err(ContractError::IoError(format!("ResolveClaimWithCapabilityUpdateV1: {} claim, {} coverage and {} underwriter bytes do not fit {} total", cn, vn, un, data.len()))); } Some(data[vpos+5..].to_vec()) } else { if data.len() != vpos + 1 { return Err(ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: trailing bytes after an absent underwriter".into())); } None }; Ok(ResolveClaimWithCapabilityUpdateV1 { claim_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[0..32].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: invalid claim_id".into()))?, coverage_id: Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: invalid coverage_id".into()))?, is_valid: data[64] != 0, payout_amount: u64::from_le_bytes(data[65..73].try_into().unwrap()), slash_amount: u64::from_le_bytes(data[73..81].try_into().unwrap()), resolved_at: u64::from_le_bytes(data[81..89].try_into().unwrap()), oracle_signature: Option::<pallas::Base>::from(pallas::Base::from_repr(data[89..121].try_into().unwrap())).ok_or_else(|| ContractError::IoError("ResolveClaimWithCapabilityUpdateV1: invalid oracle_signature".into()))?, claim_bytes: data[125..cpos].to_vec(), coverage_bytes: data[cpos+4..vpos].to_vec(), underwriter_bytes }) } }
