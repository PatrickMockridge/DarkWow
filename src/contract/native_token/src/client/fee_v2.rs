...
impl FeeV2CallBuilder {
    ...
    pub fn build(self) -> Result<FeeV2Result, ContractError> {
        ...
        // Compute tx_binding with threshold binding (spec §5.5)
...
fn create_fee_v3_proof(
    ...
    // Build witnesses (matching circuit witness order)
    ...
    for i in 0..32 {
        let sibling = if i < input.merkle_path.len() {
            input.merkle_path[i].inner()
        } else {
            pallas::Base::zero()
        };
        witnesses.push(Witness::Base(Value::known(sibling)));
    }
    witnesses.push(Witness::Base(Value::known(*input.ephemeral_signature_secret.inner())));
    witnesses.push(Witness::Base(Value::known(sig_x)));
    witnesses.push(Witness::Base(Value::known(sig_y)));
    witnesses.push(Witness::Base(Value::known(pallas::Base::from(input.value))));
    witnesses.push(Witness::Scalar(Value::known(input_value_blind.inner())));
    witnesses.push(Witness::Base(Value::known(input.spend_hook)));
    witnesses.push(Witness::Base(Value::known(input.user_data)));
    witnesses.push(Witness::Base(Value::known(input.coin_blind)));
    witnesses.push(Witness::Base(Value::known(pallas::Base::zero()))); // input_user_data_blind
    witnesses.push(Witness::Base(Value::known(pallas::Base::from(output_value))));
    witnesses.push(Witness::Base(Value::known(output.spend_hook)));
    witnesses.push(Witness::Base(Value::known(output.user_data)));
    witnesses.push(Witness::Scalar(Value::known(output_value_blind.inner())));
    witnesses.push(Witness::Base(Value::known(output.coin_blind)));
    witnesses.push(Witness::Base(Value::known(input.token_id)));
    witnesses.push(Witness::Base(Value::known(token_blind.inner())));
    witnesses.push(Witness::Base(Value::known(pallas::Base::from(fee_amount))));
    witnesses.push(Witness::Base(Value::known(tx_commitment)));
    witnesses.push(Witness::Base(Value::known(input.tx_nonce)));
    witnesses.push(Witness::Base(Value::known(pallas::Base::zero()))); // tx_binding placeholder
    witnesses.push(Witness::Scalar(Value::known(fee_value_blind.inner())));
