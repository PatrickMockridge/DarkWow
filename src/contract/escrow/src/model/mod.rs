...
impl Escrow {
    /// Derive the escrow ID from buyer_pubkey, seller_pubkey, value, asset_id, and timeout
    #[allow(dead_code)]
    pub fn derive_id(
...
impl Escrow {
    ...
    ///   value(8) + asset_id(32) + timeout(8) + state(1) +
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ContractError> {
        ...
        let asset_id = Option::<pallas::Base>::from(
            pallas::Base::from_repr(data[105..137].try_into().unwrap())
        ).ok_or_else(|| ContractError::IoError("Escrow: invalid asset_id".into()))?;
...
impl dwow_serial::Encodable for FundEscrowParamsV1 { fn encode<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<usize> { let b = self.encode().map_err(|e| std::io::Error::other(format!("{e}")))?; w.write_all(&b)?; Ok(b.len()) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl FundEscrowParamsV1 { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let n = SerializedLen::try_from_len(self.merkle_proof.len())?; let mut b = Vec::with_capacity(68+self.merkle_proof.len()*32); b.extend_from_slice(&self.escrow_id.encode()); b.extend_from_slice(&self.value_commit.to_bytes()); b.extend_from_slice(&n.to_le_bytes()); for p in &self.merkle_proof { b.extend_from_slice(&p.to_repr()); } b.extend_from_slice(&self.merkle_root.to_bytes()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 68 { return Err(ContractError::IoError("FundEscrowParamsV1: too short".into())); } let escrow_id = EscrowId::decode(&data[0..32])?; let value_commit = Option::<pallas::Point>::from(pallas::Point::from_bytes(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("FundEscrowParamsV1: invalid value_commit".into()))?; let mp_count = SerializedLen::from_le_bytes(data[64..68].try_into().unwrap()).to_usize(); let mp_end = mp_count.saturating_mul(32).saturating_add(68); if data.len() < mp_end.saturating_add(32) { return Err(ContractError::IoError("FundEscrowParamsV1: merkle_proof truncated".into())); } let mut merkle_proof = Vec::with_capacity(mp_count); for i in 0..mp_count { merkle_proof.push(Option::<pallas::Base>::from(pallas::Base::from_repr(data[68+i*32..68+(i+1)*32].try_into().unwrap())).ok_or_else(|| ContractError::IoError(format!("FundEscrowParamsV1: invalid merkle_proof[{}]", i)))?); } let merkle_root = MerkleNode::from_bytes(data[mp_end..mp_end+32].try_into().unwrap()).ok_or_else(|| ContractError::IoError("FundEscrowParamsV1: invalid merkle_root".into()))?; Ok(FundEscrowParamsV1 { escrow_id, value_commit, merkle_proof, merkle_root }) } }
