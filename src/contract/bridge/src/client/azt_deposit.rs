...
impl AztDepositCallData {
    ...
    /// Compute commitment: H(DOMAIN_COIN_COMMIT, secret, value, bridge_address)
    pub fn compute_commitment(&self) -> pallas::Base {
        let bridge_address = self.derive_bridge_address();
        poseidon_hash([pallas::Base::from(4u64), self.secret, pallas::Base::from(self.value), bridge_address])
    }
