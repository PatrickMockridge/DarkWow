...
use dwow_sdk::{blockchain::SerializedLen, crypto::pasta_prelude::PrimeField, error::ContractResult, error::ContractError, pasta::pallas};
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ChainData { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { match self { Self::Ethereum{contract,log_index}=>{let mut b=Vec::with_capacity(29);b.push(0);b.extend_from_slice(contract);b.extend_from_slice(&log_index.to_le_bytes());Ok(b)} Self::Monero{tx_hash,output_index,amount}=>{let mut b=Vec::with_capacity(49);b.push(1);b.extend_from_slice(tx_hash);b.extend_from_slice(&output_index.to_le_bytes());b.extend_from_slice(&amount.to_le_bytes());Ok(b)} Self::Zcash{nullifier,commitment,anchor}=>{let mut b=Vec::with_capacity(97);b.push(2);b.extend_from_slice(nullifier);b.extend_from_slice(commitment);b.extend_from_slice(anchor);Ok(b)} Self::Aztec{nullifier,commitment,proof_bytes}=>{let mut b=Vec::with_capacity(69+proof_bytes.len());b.push(3);b.extend_from_slice(nullifier);b.extend_from_slice(commitment);b.extend_from_slice(&SerializedLen::try_from_len(proof_bytes.len())?.to_le_bytes());b.extend_from_slice(proof_bytes);Ok(b)} Self::Litecoin{tx_hash,output_index,is_confidential}=>{let mut b=Vec::with_capacity(42);b.push(4);b.extend_from_slice(tx_hash);b.extend_from_slice(&output_index.to_le_bytes());b.push(*is_confidential as u8);Ok(b)} } } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.is_empty() { return Err(ContractError::IoError("ChainData: empty".into())); } match data[0] { 0=>{ if data.len()!=29 { return Err(ContractError::IoError("ChainData::Ethereum: bad len".into())); } Ok(Self::Ethereum{contract:data[1..21].try_into().unwrap(),log_index:u64::from_le_bytes(data[21..29].try_into().unwrap())}) } 1=>{ if data.len()!=49 { return Err(ContractError::IoError("ChainData::Monero: bad len".into())); } Ok(Self::Monero{tx_hash:data[1..33].try_into().unwrap(),output_index:u64::from_le_bytes(data[33..41].try_into().unwrap()),amount:u64::from_le_bytes(data[41..49].try_into().unwrap())}) } 2=>{ if data.len()!=97 { return Err(ContractError::IoError("ChainData::Zcash: bad len".into())); } Ok(Self::Zcash{nullifier:data[1..33].try_into().unwrap(),commitment:data[33..65].try_into().unwrap(),anchor:data[65..97].try_into().unwrap()}) } 3=>{ if data.len()<69 { return Err(ContractError::IoError("ChainData::Aztec: too short".into())); } let pb_len=SerializedLen::from_le_bytes(data[65..69].try_into().unwrap()).to_usize(); let expected=pb_len.saturating_add(69); if data.len()!=expected { return Err(ContractError::IoError("ChainData::Aztec: bad len".into())); } Ok(Self::Aztec{nullifier:data[1..33].try_into().unwrap(),commitment:data[33..65].try_into().unwrap(),proof_bytes:data[69..expected].to_vec()}) } 4=>{ if data.len()!=42 { return Err(ContractError::IoError("ChainData::Litecoin: bad len".into())); } Ok(Self::Litecoin{tx_hash:data[1..33].try_into().unwrap(),output_index:u64::from_le_bytes(data[33..41].try_into().unwrap()),is_confidential:data[41]!=0}) } _=>Err(ContractError::IoError(format!("ChainData: unknown variant {}",data[0]))) } } }

/// Deposit parameters from external chain
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl ExternalDeposit { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mp = self.merkle_proof.encode()?; let fp = match self.finality_proof.as_ref() { Some(f) => Some(f.encode()?), None => None }; let cd = self.chain_data.encode()?; let mut b = Vec::with_capacity(74+mp.len()+fp.as_ref().map_or(1,|v| 1+v.len())+cd.len()); b.push(self.chain.as_u8()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.recipient_cap); b.extend_from_slice(&self.block_hash); b.extend_from_slice(&SerializedLen::try_from_len(mp.len())?.to_le_bytes()); b.extend_from_slice(&mp); b.push(self.finality_proof.is_some() as u8); if let Some(ref fb) = fp { b.extend_from_slice(&SerializedLen::try_from_len(fb.len())?.to_le_bytes()); b.extend_from_slice(fb); } b.extend_from_slice(&SerializedLen::try_from_len(cd.len())?.to_le_bytes()); b.extend_from_slice(&cd); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 78 { return Err(ContractError::IoError("ExternalDeposit: too short".into())); } let chain = ChainId::decode(&data[0..1])?; let amount = u64::from_le_bytes(data[1..9].try_into().unwrap()); let recipient_cap: [u8;32] = data[9..41].try_into().unwrap(); let block_hash: [u8;32] = data[41..73].try_into().unwrap(); let mp_len = SerializedLen::from_le_bytes(data[73..77].try_into().unwrap()).to_usize(); let mp_end = mp_len.saturating_add(77); if data.len() < mp_end { return Err(ContractError::IoError("ExternalDeposit: mp truncated".into())); } let merkle_proof = MerkleProof::decode(&data[77..mp_end])?; let mut pos = mp_end; if data.len() < pos+1 { return Err(ContractError::IoError("ExternalDeposit: fp flag missing".into())); } let has_fp = data[pos] != 0; pos += 1; let finality_proof = if has_fp { if data.len() < pos+4 { return Err(ContractError::IoError("ExternalDeposit: fp len truncated".into())); } let fp_len = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4; if data.len() < pos.saturating_add(fp_len) { return Err(ContractError::IoError("ExternalDeposit: fp truncated".into())); } let fp = FinalityProof::decode(&data[pos..pos+fp_len])?; pos += fp_len; Some(fp) } else { None }; if data.len() < pos+4 { return Err(ContractError::IoError("ExternalDeposit: cd len missing".into())); } let cd_len = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4; if data.len() < pos.saturating_add(cd_len) { return Err(ContractError::IoError("ExternalDeposit: chain_data truncated".into())); } let chain_data = ChainData::decode(&data[pos..pos+cd_len])?; Ok(ExternalDeposit { chain, amount, recipient_cap, block_hash, merkle_proof, finality_proof, chain_data }) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl VerifiedWithdrawal { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mut b = Vec::with_capacity(53+self.recipient_address.len()); b.push(self.chain.as_u8()); b.extend_from_slice(&self.nullifier); b.extend_from_slice(&SerializedLen::try_from_len(self.recipient_address.len())?.to_le_bytes()); b.extend_from_slice(&self.recipient_address); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&self.fee.to_le_bytes()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 53 { return Err(ContractError::IoError("VerifiedWithdrawal: too short".into())); } let chain = ChainId::decode(&data[0..1])?; let nullifier: [u8;32] = data[1..33].try_into().unwrap(); let addr_len = SerializedLen::from_le_bytes(data[33..37].try_into().unwrap()).to_usize(); let addr_end = addr_len.saturating_add(37); if data.len() < addr_end.saturating_add(16) { return Err(ContractError::IoError("VerifiedWithdrawal: truncated".into())); } Ok(VerifiedWithdrawal { chain, nullifier, recipient_address: data[37..addr_end].to_vec(), amount: u64::from_le_bytes(data[addr_end..addr_end+8].try_into().unwrap()), fee: u64::from_le_bytes(data[addr_end+8..addr_end+16].try_into().unwrap()) }) } }
...
#[expect(clippy::unwrap_used, reason = "slice length checked above")]
impl HtlcSwap { pub fn encode(&self) -> Result<Vec<u8>, ContractError> { let mut b = Vec::with_capacity(88+self.external_sender.len()+self.external_recipient.len()); b.extend_from_slice(&self.swap_id); b.extend_from_slice(&self.hash.to_repr()); b.extend_from_slice(&self.timelock.to_le_bytes()); b.extend_from_slice(&self.amount.to_le_bytes()); b.extend_from_slice(&SerializedLen::try_from_len(self.external_sender.len())?.to_le_bytes()); b.extend_from_slice(&self.external_sender); b.extend_from_slice(&SerializedLen::try_from_len(self.external_recipient.len())?.to_le_bytes()); b.extend_from_slice(&self.external_recipient); b.push(self.state as u8); b.extend_from_slice(&self.created_at.to_le_bytes()); Ok(b) } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 88 { return Err(ContractError::IoError("HtlcSwap: too short".into())); } let swap_id: [u8;32] = data[0..32].try_into().unwrap(); let hash = Option::<pallas::Base>::from(pallas::Base::from_repr(data[32..64].try_into().unwrap())).ok_or_else(|| ContractError::IoError("HtlcSwap: invalid hash".into()))?; let timelock = u64::from_le_bytes(data[64..72].try_into().unwrap()); let amount = u64::from_le_bytes(data[72..80].try_into().unwrap()); let sender_len = SerializedLen::from_le_bytes(data[80..84].try_into().unwrap()).to_usize(); let mut pos = sender_len.saturating_add(84); if data.len() < pos+4 { return Err(ContractError::IoError("HtlcSwap: sender truncated".into())); } let external_sender = data[84..pos].to_vec(); let recip_len = SerializedLen::from_le_bytes(data[pos..pos+4].try_into().unwrap()).to_usize(); pos += 4; if data.len() < pos.saturating_add(recip_len).saturating_add(1+8) { return Err(ContractError::IoError("HtlcSwap: recipient truncated".into())); } let external_recipient = data[pos..pos+recip_len].to_vec(); pos += recip_len; let state = HtlcState::decode(&data[pos..pos+1])?; pos += 1; let created_at = u64::from_le_bytes(data[pos..pos+8].try_into().unwrap()); Ok(HtlcSwap { swap_id, hash, timelock, amount, external_sender, external_recipient, state, created_at }) } }
