...
pub struct CreateSwapParams {
    ...
    /// Leaf position of the lock commitment in the trusted coin tree
...
impl AcceptSwapParams { pub fn encode(&self) -> Vec<u8> { let mut b = Vec::with_capacity(138+self.lock_proof.len()*32); b.extend_from_slice(&self.swap_id); b.extend_from_slice(&self.lock_commitment.to_bytes()); b.extend_from_slice(&self.nullifier.to_bytes()); b.push(self.lock_proof.len() as u8); for h in &self.lock_proof { b.extend_from_slice(h); } b.extend_from_slice(&self.signature_public.to_bytes()); b.extend_from_slice(&self.fee.to_le_bytes()); b.push(self.immediate_execute as u8); b.extend_from_slice(&self.leaf_position.to_le_bytes()); b } pub fn decode(data: &[u8]) -> Result<Self, ContractError> { if data.len() < 145 { return Err(ContractError::IoError("AcceptSwapParams: too short".into())); } let swap_id: [u8;32] = data[0..32].try_into().unwrap(); let lock_commitment = IntentCommitment::from_bytes(data[32..64].try_into().unwrap()).map_err(|_| ContractError::IoError("AcceptSwapParams: invalid lock_commitment".into()))?; let nullifier = IntentNullifier::from_bytes(data[64..96].try_into().unwrap()).map_err(|_| ContractError::IoError("AcceptSwapParams: invalid nullifier".into()))?; let lp_count = data[96] as usize; let lp_end = 97+lp_count*32; if data.len() < lp_end+32+8+1+8 { return Err(ContractError::IoError("AcceptSwapParams: lock_proof truncated".into())); } let mut lock_proof = Vec::with_capacity(lp_count); for i in 0..lp_count { lock_proof.push(data[97+i*32..97+(i+1)*32].try_into().unwrap()); } let signature_public = PublicKey::from_bytes(data[lp_end..lp_end+32].try_into().unwrap()).map_err(|e| ContractError::IoError(format!("AcceptSwapParams: invalid signature_public: {}", e)))?; let fee = u64::from_le_bytes(data[lp_end+32..lp_end+40].try_into().unwrap()); let immediate_execute = data[lp_end+40] != 0; let leaf_position = u64::from_le_bytes(data[lp_end+41..lp_end+49].try_into().unwrap()); Ok(AcceptSwapParams { swap_id, lock_commitment, nullifier, lock_proof, signature_public, fee, immediate_execute, leaf_position }) } }
