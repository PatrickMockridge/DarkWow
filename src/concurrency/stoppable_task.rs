...
impl StoppableTask {
        ...
        Error: std::error::Error + Send + 'a,
    {
        ...
        trace!(target: "concurrency::StoppableTask", "Starting task {}", self.task_id);
        ...
        executor
            .spawn(async move {
                ...
                let stop_fut = async {
                    ...
                    trace!(
                        target: "concurrency::StoppableTask",
    ...
    pub fn stop_nowait(&self) {
        trace!(target: "concurrency::StoppableTask", "Stopping task (nowait) {}", self.task_id);
        self.signal.notify();
    }
}
