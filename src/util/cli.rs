...
/// Example usage:
/// ```
/// use dwow_core::{async_daemonize, cli_desc, Result};
...
macro_rules! async_daemonize {
    ($realmain:ident) => {
        fn main() -> Result<()> {
            ...
            let cfg_path =
                match dwow_core::util::path::get_config_path(args.config.clone(), CONFIG_FILE) {
                    Ok(v) => v,
                    Err(e) => {
            ...
            let (non_blocking, file_guard) = match args.log {
                Some(ref log_path) => {
                    let log_path = match dwow_core::util::path::expand_path(log_path) {
                        Ok(v) => v,
                        Err(e) => {
            ...
            if let Err(e) = dwow_core::util::logger::setup_logging(args.verbose, non_blocking) {
                if args.log.is_some() {
            ...
            while let Some(signal) = signals.next().await {
                match signal {
                    signal_hook::consts::SIGHUP => {
                        ...
                        let cfg_path =
                            dwow_core::util::path::get_config_path(args.config, CONFIG_FILE)?;
                        dwow_core::util::cli::spawn_config(
                            &cfg_path,
