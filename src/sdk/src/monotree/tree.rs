...
impl<D: MonotreeStorageAdapter> Monotree<D> {
    ...
    fn put(&mut self, root: &[u8], bits: Bits, leaf: &[u8]) -> GenericResult<Option<Hash>> {
        let bytes = self.db.get(root)?.ok_or(ContractError::MonotreeError(
            "put(): bytes".to_string(),
        ))?;
        let (left, right) = Node::cells_from_bytes(&bytes, bits.first())?;
        let unit = left.as_ref().ok_or(ContractError::MonotreeError(
            "put(): left-unit".to_string(),
        ))?;
        ...
        match n {
            ...
            _ => {
                let hash = &self
                    ...
                    .ok_or(ContractError::MonotreeError(
                        "put(): split-node".to_string(),
                    ))?;
    ...
    fn find_key(&mut self, root: &[u8], bits: Bits) -> GenericResult<Option<Hash>> {
        let bytes = self.db.get(root)?.ok_or(ContractError::MonotreeError(
            "find_key(): bytes".to_string(),
        ))?;
        let (cell, _) = Node::cells_from_bytes(&bytes, bits.first())?;
        let unit = cell.as_ref().ok_or(ContractError::MonotreeError(
            "find_key(): left-unit".to_string(),
        ))?;
        let n = Bits::len_common_bits(&unit.bits, &bits);
        match n {
    ...
    fn delete_key(&mut self, root: &[u8], bits: Bits) -> GenericResult<Option<Hash>> {
        let bytes = self.db.get(root)?.ok_or(ContractError::MonotreeError(
            "delete_key(): bytes".to_string(),
        ))?;
        let (left, right) = Node::cells_from_bytes(&bytes, bits.first())?;
        let unit = left.as_ref().ok_or(ContractError::MonotreeError(
            "delete_key(): left-unit".to_string(),
        ))?;
        let n = Bits::len_common_bits(&unit.bits, &bits);
    ...
    fn gen_proof(
        ...
        let bytes = self.db.get(root)?.ok_or(ContractError::MonotreeError(
            "gen_proof(): bytes".to_string(),
        ))?;
        let (cell, _) = Node::cells_from_bytes(&bytes, bits.first())?;
        let unit = cell.as_ref().ok_or(ContractError::MonotreeError(
            "gen_proof(): left-unit".to_string(),
        ))?;
        let n = Bits::len_common_bits(&unit.bits, &bits);
...
pub fn verify_proof(
    ...
    match proof {
        ...
        Some(proof) => {
            ...
            let expected_root = root.ok_or(ContractError::MonotreeError(
                "verify_proof(): root".to_string(),
            ))?;
            Ok(expected_root == &hash)
        }
    }
}
