...
/// [`insurance_market_model::FileClaimParamsV1`] python binding.
#[pyclass]
pub struct InsuranceMarketFileClaimParamsV1(insurance_market_model::FileClaimParamsV1);
impl_py_methods!(InsuranceMarketFileClaimParamsV1);

impl FunctionParams for insurance_market_model::FileClaimParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("coverage_id", format!("{:?}", self.coverage_id))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}coverage_id: {:?}", self.coverage_id).unwrap();
