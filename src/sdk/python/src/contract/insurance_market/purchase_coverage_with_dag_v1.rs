...
/// [`insurance_market_model::PurchaseCoverageWithDAGParamsV1`] python binding.
#[pyclass]
pub struct InsuranceMarketPurchaseCoverageWithDAGParamsV1(
    insurance_market_model::PurchaseCoverageWithDAGParamsV1,
);
impl_py_methods!(InsuranceMarketPurchaseCoverageWithDAGParamsV1);

impl FunctionParams for insurance_market_model::PurchaseCoverageWithDAGParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        dict.set_item("dag_proof", format!("{:?}", self.dag_proof))?;
        dict.set_item("dag_path_index", format!("{:?}", self.dag_path_index))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}dag_proof: {:?}", self.dag_proof).unwrap();
        writeln!(out, "{prefix}dag_path_index: {:?}", self.dag_path_index).unwrap();
