...
/// [`dao_escrow_model::ResolveDisputeParamsV1`] python binding.
#[pyclass]
pub struct DaoEscrowResolveDisputeParamsV1(dao_escrow_model::ResolveDisputeParamsV1);
impl_py_methods!(DaoEscrowResolveDisputeParamsV1);

impl FunctionParams for dao_escrow_model::ResolveDisputeParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        dict.set_item("attestations", format!("{:?}", self.attestations))?;
        dict.set_item("capability_proof", format!("{:?}", self.capability_proof))?;
        dict.set_item("payout_amount", format!("{:?}", self.payout_amount))?;
        dict.set_item("payout_recipient", self.payout_recipient.to_string())?;
        Ok(dict.unbind())
    }
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}attestations: {:?}", self.attestations).unwrap();
        ...
        writeln!(out, "{prefix}payout_recipient: {}", self.payout_recipient).unwrap();
        Ok(())
    }
}
