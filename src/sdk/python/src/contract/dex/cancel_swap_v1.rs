...
/// [`dex_model::CancelSwapParams`] python binding.
#[pyclass]
pub struct DexCancelSwapParamsV1(dex_model::CancelSwapParams);
impl_py_methods!(DexCancelSwapParamsV1);

impl FunctionParams for dex_model::CancelSwapParams {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("swap_id", format!("{:?}", self.swap_id))?;
        dict.set_item("secret", format!("{:?}", self.secret))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}secret: {:?}", self.secret).unwrap();
