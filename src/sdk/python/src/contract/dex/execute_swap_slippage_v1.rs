...
/// [`dex_model::ExecuteSwapSlippageParams`] python binding.
#[pyclass]
pub struct DexExecuteSwapSlippageParamsV1(dex_model::ExecuteSwapSlippageParams);
impl_py_methods!(DexExecuteSwapSlippageParamsV1);

impl FunctionParams for dex_model::ExecuteSwapSlippageParams {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        dict.set_item("slippage_bps", format!("{:?}", self.slippage_bps))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}slippage_bps: {:?}", self.slippage_bps).unwrap();
