...
/// [`dex_model::CreateSwapParams`] python binding.
#[pyclass]
pub struct DexCreateSwapParamsV1(dex_model::CreateSwapParams);
impl_py_methods!(DexCreateSwapParamsV1);

impl FunctionParams for dex_model::CreateSwapParams {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("swap_id", format!("{:?}", self.swap_id))?;
        dict.set_item("offer_token", format!("{:?}", self.offer_token))?;
        dict.set_item("offer_amount", format!("{:?}", self.offer_amount))?;
        dict.set_item("request_token", format!("{:?}", self.request_token))?;
        dict.set_item("request_amount", format!("{:?}", self.request_amount))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}offer_token: {:?}", self.offer_token).unwrap();
        writeln!(out, "{prefix}offer_amount: {:?}", self.offer_amount).unwrap();
        writeln!(out, "{prefix}request_token: {:?}", self.request_token).unwrap();
        writeln!(out, "{prefix}request_amount: {:?}", self.request_amount).unwrap();
