...
/// [`baccarat_model::CommitBetParamsV1`] python binding.
#[pyclass]
pub struct BaccaratCommitBetParamsV1(baccarat_model::CommitBetParamsV1);
impl_py_methods!(BaccaratCommitBetParamsV1);

impl FunctionParams for baccarat_model::CommitBetParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("player_pub", format!("{:?}", self.player_pub))?;
        dict.set_item("bet_type", self.bet_type)?;
