...
/// [`pool_stake_model::AllocateCoverageParamsV1`] python binding.
#[pyclass]
pub struct PoolStakeAllocateCoverageParamsV1(pool_stake_model::AllocateCoverageParamsV1);
impl_py_methods!(PoolStakeAllocateCoverageParamsV1);

impl FunctionParams for pool_stake_model::AllocateCoverageParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        dict.set_item("withdrawal_id", format!("{:?}", self.withdrawal_id))?;
        dict.set_item("nonce", format!("{:?}", self.nonce))?;
        dict.set_item("derived_allocation_id", format!("{:?}", self.derived_allocation_id))?;
        Ok(dict.unbind())
    }
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}withdrawal_id: {:?}", self.withdrawal_id).unwrap();
        writeln!(out, "{prefix}nonce: {:?}", self.nonce).unwrap();
        writeln!(out, "{prefix}derived_allocation_id: {:?}", self.derived_allocation_id).unwrap();
        Ok(())
    }
}
