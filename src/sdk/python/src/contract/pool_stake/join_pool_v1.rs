...
/// [`pool_stake_model::JoinPoolParamsV1`] python binding.
#[pyclass]
pub struct PoolStakeJoinPoolParamsV1(pool_stake_model::JoinPoolParamsV1);
impl_py_methods!(PoolStakeJoinPoolParamsV1);

impl FunctionParams for pool_stake_model::JoinPoolParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        dict.set_item("value_commit_y", format!("{:?}", self.value_commit_y))?;
        Ok(dict.unbind())
    }
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}value_commit_y: {:?}", self.value_commit_y).unwrap();
        Ok(())
    }
}
