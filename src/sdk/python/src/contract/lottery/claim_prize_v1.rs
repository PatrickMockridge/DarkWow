...
/// [`lottery_model::ClaimPrizeParamsV1`] python binding.
#[pyclass]
pub struct LotteryClaimPrizeParamsV1(lottery_model::ClaimPrizeParamsV1);
impl_py_methods!(LotteryClaimPrizeParamsV1);

impl FunctionParams for lottery_model::ClaimPrizeParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("ticket_id", format!("{:?}", self.ticket_id))?;
        dict.set_item("proof", format!("{:?}", self.proof))?;
        dict.set_item("tier", self.tier)?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}tier: {}", self.tier).unwrap();
