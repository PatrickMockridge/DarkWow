...
/// [`bridge_model::GovernanceReportParams`] python binding.
#[pyclass]
pub struct BridgeGovernanceReportParamsV1(bridge_model::GovernanceReportParams);
impl_py_methods!(BridgeGovernanceReportParamsV1);

impl FunctionParams for bridge_model::GovernanceReportParams {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("chain", format!("{:?}", self.chain))?;
        dict.set_item("total_deposited", format!("{:?}", self.total_deposited))?;
        dict.set_item("total_withdrawn", format!("{:?}", self.total_withdrawn))?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}total_deposited: {:?}", self.total_deposited).unwrap();
        writeln!(out, "{prefix}total_withdrawn: {:?}", self.total_withdrawn).unwrap();
