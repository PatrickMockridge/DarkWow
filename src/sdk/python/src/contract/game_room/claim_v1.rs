...
/// [`game_room_model::ClaimParamsV1`] python binding.
#[pyclass]
pub struct GameRoomClaimParamsV1(game_room_model::ClaimParamsV1);
impl_py_methods!(GameRoomClaimParamsV1);

impl FunctionParams for game_room_model::ClaimParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("room_id", format!("{:?}", self.room_id))?;
        dict.set_item("pot_id", format!("{:?}", self.pot_id))?;
        dict.set_item("winner", self.winner.to_string())?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}winner: {}", self.winner).unwrap();
