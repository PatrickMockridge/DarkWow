...
/// [`darkbet_exchange_model::PlaceBackParamsV1`] python binding.
#[pyclass]
pub struct DarkbetPlaceBackParamsV1(darkbet_exchange_model::PlaceBackParamsV1);
impl_py_methods!(DarkbetPlaceBackParamsV1);

impl FunctionParams for darkbet_exchange_model::PlaceBackParamsV1 {
    fn to_pydict(&self, py: Python) -> PyResult<Py<PyDict>> {
        let dict = PyDict::new(py);
        dict.set_item("market_id", format!("{:?}", self.market_id))?;
        dict.set_item("outcome_index", self.outcome_index)?;
        dict.set_item("odds", self.odds)?;
        dict.set_item("stake", self.stake)?;
    ...
    fn fmt_pretty(&self, out: &mut String, depth: usize) -> PyResult<()> {
        ...
        writeln!(out, "{prefix}outcome_index: {}", self.outcome_index).unwrap();
        writeln!(out, "{prefix}odds: {}", self.odds).unwrap();
        writeln!(out, "{prefix}stake: {}", self.stake).unwrap();
