...
impl RequestHandler<ManagementRpcHandler> for DwowNode {
    async fn handle_request(&self, req: JsonRequest) -> JsonResult {
        ...
        match req.method.as_str() {
            ...
            // Account management
            // Account management (read-only). The account is declared in keys.toml
            // and re-derived deterministically on boot — there is no mutable, random,
            // or persisted account store to list/import/generate/remove/set-default.
            "accounts.show" => self.accounts_show(req.id, req.params).await,
...
impl DwowNode {
    ...
    /// Show the node's declared account (read-only). Keys are declared in
    /// keys.toml and re-derived on boot; there is no mutable account store.
    /// --> {"jsonrpc": "2.0", "method": "accounts.show", "params": [], "id": 1}
    pub async fn accounts_show(&self, id: u16, _params: JsonValue) -> JsonResult {
        let mgr = self.account_manager.read().await;
        let pk = match mgr.default_public_key() {
            Ok(pk) => pk,
            Err(e) => return JsonError::new(ErrorCode::InternalError, Some(e), id).into(),
        };
        let address = mgr.accounts().first().map(|a| a.address(mgr.network)).unwrap_or_default();
        let mut obj = std::collections::HashMap::new();
        obj.insert("address".to_string(), JsonValue::String(address));
        obj.insert("public".to_string(), JsonValue::String(hex::encode(pk.to_bytes())));
