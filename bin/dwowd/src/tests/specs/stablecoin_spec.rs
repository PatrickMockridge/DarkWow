    ...
    ContractTestSpec { name: "stablecoin", is_genesis: false,
        ...
        endpoints: vec![
            ...
            mk_ep("AddCollateralV1", true, Box::new(move || {
                ...
                let r = h.add_collateral(&params).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
            mk_ep("RemoveCollateralV1", true, Box::new(move || {
                ...
                let r = h.remove_collateral(&params).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
            mk_ep("RepayStableV1", true, Box::new(move || {
                ...
                let r = h.repay_stable(&params).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
            mk_ep("UpdateConfigV1", true, Box::new(move || {
                ...
                let r = h.update_config(&params).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
        ],
    }
