    ...
    ContractTestSpec {
        ...
        endpoints: vec![
            mk_ep("SubscribeV1", true, Box::new(move || {
                let r = h.subscribe(sub_secret, pallas::Base::from(1u64), ep.clone(), pallas::Scalar::from(1u64), pallas::Base::from(2u64), pallas::Base::from(3u64), 1000, pallas::Base::from(4u64), 0, ep.clone(), 0, ep.clone(), subscription_id, sub_pub, 1, 5000, pallas::Base::from(5u64), 200, pallas::Base::from(6u64), 100, pallas::Base::from(7u64), pallas::Base::from(8u64), pallas::Base::from(9u64), pallas::Base::from(10u64), pallas::Base::from(11u64)).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
            mk_ep("VerifyAccessV1", true, Box::new(move || {
                let r = h.verify_access(sub_secret, pallas::Base::from(1u64), 1, 0, ep.clone(), pallas::Base::from(2u64), pallas::Base::from(3u64), pallas::Base::from(4u64), subscription_id, 100, sub_pub.x().unwrap(), sub_pub.y().unwrap(), 1, 200, 10, 3600, 5, 100, 5, pallas::Base::from(6u64)).map_err(|e| dwow_core::Error::Custom(format!("{e}")))?;
                Ok(EndpointResult { call_data: r.call_data, proofs: vec![r.proof] })
            })),
