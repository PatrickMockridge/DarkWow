...
impl TaskInfo {
    ...
    ) -> TauPallasResult<Self> {
        // generate ref_id
        let ref_id = gen_id(30);

        if let Some(d) = &due {
            if *d < Timestamp::current_time() {
                return Err(TauPallasError::InvalidDueTime)
            }
        }
    ...
    pub fn load(ref_id: &str, dataset_path: &Path) -> TauPallasResult<Self> {
    ...
    pub fn activate(&self, path: &Path) -> TauPallasResult<()> {
    ...
    pub fn deactivate(&self, path: &Path) -> TauPallasResult<()> {
