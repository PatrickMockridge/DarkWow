...
///
/// DarkWow linear header layout (from dwow_linear::BlockHeader):
/// ```text
/// Offset  Size  Field
/// 0       1     version (u8)
/// 1       32    previous (blake3::Hash)
/// 33      32    merkle_root (blake3::Hash)
/// 65      8     timestamp (u64, LE)
/// 73      4     difficulty_target (u32, LE)
/// 77      4     nonce (u32, LE)  ← miners modify this
/// 81      8     height (u64, LE)
...
pub const NONCE_OFFSET: usize = 77;

/// Serialize a DarkWow BlockHeader into a fixed-size byte array.
/// This is the `blockhashing_blob` that miners will hash with RandomX.
pub fn serialize_header(header: &BlockHeader) -> Vec<u8> {
    ...
    buf.extend_from_slice(header.previous.as_bytes());
    ...
    buf.extend_from_slice(&header.uncle_merkle_root);
    buf.extend_from_slice(&header.total_reward.to_le_bytes());
    buf.extend_from_slice(&header.randomx_key);
    buf.extend_from_slice(&header.coin_merkle_root);
    buf.extend_from_slice(&header.nullifier_root);
...
pub fn deserialize_header(data: &[u8]) -> Option<BlockHeader> {
    ...
    let version = data[0];
    let previous = blake3::Hash::from_bytes(data[1..33].try_into().ok()?);
    let merkle_root = blake3::Hash::from_bytes(data[33..65].try_into().ok()?);
    let timestamp = u64::from_le_bytes(data[65..73].try_into().ok()?);
    let difficulty_target = u32::from_le_bytes(data[73..77].try_into().ok()?);
    let nonce = u32::from_le_bytes(data[77..81].try_into().ok()?);
    let height = u64::from_le_bytes(data[81..89].try_into().ok()?);
