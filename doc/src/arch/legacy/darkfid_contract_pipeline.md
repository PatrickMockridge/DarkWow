# Contract Deployment Pipeline (Legacy DAG Architecture)

> **ARCHIVED — LEGACY DOCUMENTATION**
>
> This document describes the overlay-DAG architecture from the upstream DarkWow codebase. It is preserved for historical reference. For the current linear-master architecture, see [Contract Deployment Pipeline](../dwowd_contract_pipeline.md).

The content below describes the original `dwowd` startup sequence with 4 native contracts (Money, DAO, Deployooor, MoneyV2), the `BlockchainOverlay` consensus mechanism, and the sled-based state management. None of this applies to the current linear-master branch, which uses Uncle Merkle consensus with only 2 native contracts (NativeToken + Deployooor) and plain sled storage without overlay/diff rollback.
...
|----------|---------|------------|-------------|
| Money | `dwow_money_contract` | Hardcoded `MONEY_CONTRACT_ID` (index 0) | dwowd at startup |
| DAO | `dwow_dao_contract` | Hardcoded `DAO_CONTRACT_ID` (index 1) | dwowd at startup |
| Deployooor | `dwow_deployooor_contract` | Hardcoded `DEPLOYOOOR_CONTRACT_ID` (index 2) | dwowd at startup |
...
| Contract | Package | ContractID | Deployed By |
|----------|---------|------------|-------------|
| Stablecoin | `dwow_stablecoin_contract` | Derived from deployer's pubkey | Deployooor contract |
| Identity | `dwow_identity_contract` | Derived from deployer's pubkey | Deployooor contract |
| DEX | `dwow_dex_contract` | Derived from deployer's pubkey | Deployooor contract |
...
```bash
RUST_MIN_STACK=67108864 cargo test -p dwow_money_contract --test integration -- --ignored
```
...
### Mainnet Deployment (dwowd + dww)

| Aspect | Native Contracts | WASM Contracts |
|--------|-----------------|----------------|
| Deployment | At dwowd startup | Via Deployooor transaction |
| VK Injection | At startup (vks::inject) | Via `dww contract deploy`? |
| ContractID | Hardcoded | Derived at runtime |
