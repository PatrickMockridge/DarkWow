...
**A gap this correction does not close, recorded rather than left to be discovered.** The twenty rows
enumerated below are satisfied *by that paragraph*; their own table rows still carry **no status
marker** (`OBL-C18`, `C23`, `C24`, `C34`, `C36`, `C38`–`C41`, `C46`–`C49`, `C51`, `C59`, `C60`, `Z15`,
`T3`, `T6`, `T10`). So a reader grepping the tables for a closure marker misses **eighteen of these
twenty** rows — `Z15` and `T10` are the two that already carry one — and the register's per-row
countability is partial. The four finality rows whose
fixes were recorded only in prose — `OBL-C68`–`C71` — were given their markers on 2026-09-22; these
twenty were not, and are enumerated here so that the omission is a recorded gap with a list, not a
silent one.
