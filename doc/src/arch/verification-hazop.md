...
| OBL-Z5 | The deliberately free witnesses are enumerated and each carries its host-side obligation | `script/circuit_free_instances.txt` | **partly.** 28 entries, each naming the Rust mechanism that constrains it (file + condition), all read rather than inferred — except six added on 2026-09-20 that are different in kind and say so in the file: the bearer_bond coverage report and dex's fee are recorded as *no mechanism exists*, deliberately free rather than checked. The `tx_nonce` case turned out not to need an entry at all — see "the redundant class" below | H |
...
| OBL-Z13 | A governance datum the reports are *about* is either derived from chain state or is recorded as the reporter's claim | `bearer_bond/proof/prove_coverage.zk`; `stablecoin/proof/governance_report.zk`; `dex/proof/execute_swap_fee.zk` | **partly.** The six circuits changed on 2026-09-20 expose the values they compute (`coverage_ratio_bps`, `collateral_ratio_bps`, `fee`) instead of leaving them as witnesses the host never checked — closing a hole in which the on-chain solvency ratio was instruction data proven by nothing. What remains is the *anchor*, and it differs per contract: stablecoin's amounts are compared against the config DB (`entrypoint.rs:1363-1368`), so its exposed ratio is a function of chain state; bearer_bond's reserve amounts are off-chain and nothing can anchor them, so its coverage report is an **issuer attestation** and `is_coverage_voided` is an issuer-controlled switch; dex's fee is proven and read by no one (`execute_swap_fee.rs` reads neither `params.fee` nor `params.fee_bps`). All three are in `script/circuit_free_instances.txt` with the mechanism — or the absence of one — stated | M |
| OBL-Z15 | A note commitment means one thing across the circuit, the client and the host | `bearer_bond/proof/{blind_output,redeem}.zk`; `commitment` handling throughout `bearer_bond` | **FAILS, and it is four objects with one name.** Read on 2026-09-20: the circuit's `coin` is `poseidon_hash(4, coin_public, value, asset_id, spend_hook, user_data, commitment_blind)` (7 arguments); `CommitmentAttributes::to_commitment` — the SDK convention the client calls (`client/pay_interest.rs:190`) — is the same hash **plus `maturity_block`** (8); the host's metadata pushes `params.commitment.token_commit`, a third hash over a different domain (`poseidon_hash([2, asset_id, asset_id_blind])`); and the commitment *tree* is keyed by that third one (`entrypoint/mod.rs:1077`). No two agree, so a proof created by the client cannot verify against the metadata even in principle, and the proofs' public-input vectors disagree position-for-position as well (`redeem`: circuit 8, metadata 8 at `:355` and 6 at `:410`, client 8 in a third order). **Not exploitable — unusable**: verification cannot succeed, so nothing moves; it is a liveness defect, not a soundness one. The remedy is to fix the notation (one commitment definition, the `CommitmentAttributes` one), carry it in the params where the host cannot recompute it (its preimage holds a private blind), and re-align all three vectors | H |
| OBL-Z16 | A policy a circuit claims to enforce is one the prover cannot parametrise away | 19 instances across `oracle`, `insurance_market`, `labor_market`, `bridge`, `roulette`, `stablecoin`, `proofs/core` | **FAILS at 19 sites, catalysed after the 2026-09-20 triage of OBL-Z1's residue.** 33 unclassified became 14 with a readable host mechanism in `script/circuit_free_instances.txt` (id lookups, membership checks, a child-call capability route, a verifier-supplied epoch) and 19 open. Two of the 19 are *live*: `insurance_market/proof/{purchase_coverage,underwrite}_with_capability.zk` expose `required_capability_id` that nothing verifies — the host only checks the market *has* a capability configured, never that the caller holds it, so a capability-gated market is open to anyone (the repository's one working pattern is `labor_market`'s `Identity::VerifyCapabilityV1` child call, `entrypoint.rs:1613-1644`). The rest are inert or prover-parametrised: `bridge/withdraw`'s anti-dust `token_minimum` is a field of the withdraw params, so the prover sets the floor it must clear; `roulette/settle_bet`'s `payout` and `stablecoin`'s `report_timestamp` are exposed and read by nobody; `oracle/aggregate`'s `result`/`min_result`/`max_result` are bounded only by each other; `oracle/attest_value`'s `attestation_id` is stored without reference to the attestation contract; `labor_market/create_job`'s `attestation_id` likewise; `labor_market/milestone_payment`'s circuit is registered by no dispatch path; `proofs/core/{lead,set_v1}.zk` have no host in this repository. **The gate stays red on exactly these**, which is its design: the exception list admits a mechanism, not an excuse | H |
...
The remaining **19** are OBL-Z16, and two of them are live.
