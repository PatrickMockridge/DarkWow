...
| OBL-C68 | Anchor acquisition is off the consensus path. `stratum_submit` calls the blocking `anchor_block` directly inside the async handler **while `linear_submit_lock` is still held** (`stratum.rs:561`, guard taken at `:348`), so one slow ArDrive Turbo POST serialises every stratum submission for up to the 30-second timeout each — and the timeout is `ureq`'s `timeout_global`, a per-IO timeout rather than a wall-clock deadline. The built-in miner does this correctly via `smol::unblock` (`miner.rs:293-307`). `doc/src/arch/consensus/stratum.md:145` describes the anchor as "best-effort, non-blocking", which is false for this path. **This is the one row in this residue with no characterization test**, deliberately: the lock scope is not observable from a unit test without a stub Turbo server and a live stratum handler, and the two halves fail to different tests. The timeout half is unit-testable against a local stub that dribbles bytes — under which `timeout_global` (a per-IO timeout, not a wall-clock deadline) does not fire at 30 s — and that test is a Stage 1 deliverable beside the hermetic stub harness; the lock half is proven by the Stage 4 integration test, which asserts a second submission completes while the first's anchor call is outstanding | `stratum.rs:348`, `:561`; `caribina/anchor.rs:84-86` | H |
