...
| Consensus | `PoWRewardParamsV1` (`native_token/src/model/mod.rs:496`); §8.2 `CoinbaseTransaction` (`src/linear/src/transaction.rs:244-264`) | `ClearInput` + `Output` + `Nullifier` (+ `MiningRecipient` barbs) | `↓mine` | plaintext — `PoWRewardParamsV1` inside `ContractCall.data`; **no ZK public inputs and no proof** (the coinbase carries no circuit) |
