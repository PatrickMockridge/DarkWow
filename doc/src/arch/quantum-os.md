...
|-----------|-----------|---------|
| **Declare currency** | `cap:token-USD:hex` — ZFA-balanced issuer authority token, stored in `currencyTokens` | `RegisterTypeV1` (0x00) on [Promissory Note](../contract/promissory_note.md) contract — registers token ID, issuer commitment, Pedersen generator |
| **Mint** | `/note grant USD 5` → `cap:note-USD:hex` where denomination = `hex.length / 2` (each twist pair = 1 unit) | `IssueV1` (0x02) → `Commitment = poseidon_hash(pub, value, asset_id, spend_hook, user_data, blind)` committed to Merkle tree |
