# Promissory Note Migration: Privacy-First DeFi Token Contract

## Executive Summary

This document describes the architecture of DarkWow on this fork:

- **Money V1/V2 are not used on this fork** — Replaced by Promissory Note (DeFi tokens) and NativeToken (consensus operations)
...
- **Promissory Note is a WASM contract** — DeFi tokens (ERC-20 style) with full privacy
- **Promissory Note uses Poseidon-only design** — Avoids EC operations implicated in heap corruption
...
| Promissory Note | WASM | **ACTIVE** | DeFi tokens (stablecoins, wrapped assets) |
...
Genesis Block
    ...
    └── Deployooor (DEPLOYOOOR_CONTRACT_ID)
            └── Deploy WASM contracts (Promissory Note, DAO Escrow, etc.)
```

Note: Promissory Note is a **WASM contract** deployed via Deployooor, not a native contract.
...
## Why Money V1/V2 Were Replaced with Promissory Note
...
| Promissory Note | **0** | **NO** - Poseidon-only |
...
### Promissory Note: Poseidon-Only Design

Promissory Note eliminates EC operations entirely:

```
Promissory Note Circuit Design:
┌─────────────────────────────────────────────────────────────┐
...
Money V2 vs Promissory Note token ID comparison:

| Aspect | Money V2 | Promissory Note |
|--------|----------|----------|
...
## NativeToken vs Promissory Note Separation
...
Note: Promissory Note ContractId is set at deployment time (user-deployed WASM contract).
...
The `dwow_wallet` command-line wallet supports Promissory Note with full functionality:
...
| Rule | Old | New |
|------|-----|-----|
| Money contract | Money V1/V2 | **Promissory Note** (WASM) |
...
- [Promissory Note Contract](../dev/contracts/promissory_note.md) - Detailed Promissory Note specification
...
- **2026-04-13**: Promissory Note migration complete
  - Money V1/V2 removed, Promissory Note added (WASM)
  ...
  - dwow_wallet wallet updated with full Promissory Note support
