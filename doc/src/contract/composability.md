...
│  │   └── children_indexes: []                                    │
│  │                                                               │
│  child[2] (money_v3::TransferV1 0x04)                             │
│      └── children_indexes: []                                    │
│                                                                   │
...
|---|---|---|---|
| `CreateJobV1` | 0x00 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `AcceptJobV1` | 0x01 | `child.data[0] != 0x04` | money_v3::TransferV1 |
...
| `ConfirmDeliveryV1` | 0x04 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `DisputeV1` | 0x05 | `child.data[0] != 0x07` | DAO-Escrow::ProposeClaimV1 |
| `RefundV1` | 0x06 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `CancelV1` | 0x07 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `CreateJobWithMilestonesV1` | 0x08 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `InitiateDisputeV1` | 0x0b | `child.data[0] != 0x07` | DAO-Escrow::ProposeClaimV1 |
| `CreateJobWithCapabilityV1` | 0x0c | `child.data[0] != 0x04` | money_v3::TransferV1 |
...
| Source Function | Opcode | Child Validates | Target Function |
|---|---|---|---|
| `PayPremiumV1` | 0x02 | `child.data[0] != 0x04` | money_v3::TransferV1 |
| `VerifyMemberCapabilityV1` | 0x0b | `child.data[0] != 0x0b` | Identity::VerifyCapabilityV1 |
| `ResolveDisputeV1` | 0x0c | `!children_indexes.is_empty()` | Attestation + money_v3 (multi-call) |
...
| money_v3 | `TransferV1` | 0x04 |

Note: Attestation::VerifyClaimV1 and money_v3::TransferV1 both use opcode 0x04. Contracts distinguish them by `contract_id` in the child call's `ContractCall` struct, not by opcode alone. The calling contract only validates the function code byte; the transaction builder sets the correct `contract_id` for each child.
...
│       data: ContractCall { contract_id: money_v3_cid, data: [0x04, ...TransferParams] }
│       children_indexes: []
│   }
│
