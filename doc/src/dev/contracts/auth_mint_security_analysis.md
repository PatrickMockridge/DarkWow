...
│   │             asset_id is in registry (Merkle proof)"
│   │  nullifier = poseidon_hash(mint_secret, asset_id)
...
asset_id = poseidon_hash(token_auth_parent, user_data, blind)   // binds authority into token ID
```
...
nullifier = poseidon_hash(mint_secret, asset_id)   // public input, one-shot
root = merkle_root(leaf_pos, path, asset_id)       // public input, proves registry membership
```
...
1. Verifies `asset_id` exists in the token registry (line 445)
...
The nullifier is **deterministic**: `poseidon_hash(mint_secret, asset_id)` is always the same for the same inputs. This means each `(mint_secret, asset_id)` pair can only be exercised once — the SMT duplicate check rejects subsequent attempts.

### Capability Revocation

**The auth_mint capability cannot be revoked.** Once a nullifier is written to the SMT, no mechanism removes it. The `mint_secret` holder's authority is permanent for that `asset_id`. If the secret is compromised, the only mitigation is to create a new `asset_id` and migrate all holders — a catastrophic UX event for a stablecoin or wrapped asset.
...
| H3 | Authority mints once then loses/compromises mint_secret — future liquidity requires new asset_id | Medium | Key mgmt |
...
| H7 | Deterministic nullifier enforces one AuthTokenMintV1 per (secret, asset_id) — multi-mint requires secret management | Medium | Usability |
...
| **BEFORE** | AuthTokenMintV1 without prior TokenMintV1 | `asset_id` not in registry → `TokenNotRegistered`. Safe. |
...
Sybil observes `token_auth_parent` and `asset_id` on-chain. She tries to call `AuthTokenMintV1` with a fabricated proof.
...
Eve observes `token_auth_parent` in every `TokenMintV1` call. She builds a mapping of `token_auth_parent → [asset_ids]`, linking every token to its authority. She can determine exactly how many token types each authority controls.
...
Eve observes that two different `asset_id`s have `token_auth_parent` values that, when hashed together, produce a recognizable pattern. She attempts to correlate authorities across tokens.
