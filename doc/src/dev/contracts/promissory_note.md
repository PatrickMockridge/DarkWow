...
DarkWow splits token functionality across two contracts:
...
to minimize consensus attack surface. Promissory Note carries the minimum viable
business logic for DeFi: token creation, minting with backing capability proofs,
private transfers, and OTC swaps.

See [NativeToken](native_token.md) for the full rationale.
