...
- [ ] **Nullifier zero-rejection**: Every nullifier path rejects `Nullifier::zero()`. A zero nullifier matches every coin. See safety.md lesson #1.
...
- [ ] **Structural conservation is not enough**: 1-in-1-out coin structure does not imply value conservation. The circuit must explicitly constrain input.value == output.value (or the intended transformation).
...
- [ ] **Opaque field audit**: Fields committed into coin hashes or passed as ZK public inputs do not carry identity-derived data. Authorization goes in nullifiers, not auxiliary data. See safety.md lesson #8.
