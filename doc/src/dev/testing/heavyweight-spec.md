...
4. **FeeCollectV1 presence:** Every block **generated by this harness** SHALL include FeeCollectV1.
   This is a harness convention, not the chain rule — see §3.5.
...
**Criterion:** Every block **generated by this harness** SHALL include FeeCollectV1.
`with_fee_collect()` SHALL NOT silently skip when no FeeV3 calls exist.

This convention is deliberately stronger than the chain rule, and reading it as consensus would be
wrong. Consensus requires the collection to be present **iff** the block has fee calls (`OBL-C117`),
so a block with no fees is valid without one, and a collection claiming zero is rejected
(`ZeroFeeClaim`). The harness therefore arranges the precondition instead of the collection: it
appends a non-zero FeeV3 fee (`add_fee`, `bin/dwowd/src/tests/modules/coinbase_coordination.rs:132`)
before collecting, so every block it builds has fees and a collection that can be settled.
