...
> **Note**: This DEP describes the original upstream `Money` contract. In DarkWow, the `Money` contract was split into `native_token` (consensus) and `promissory_note` (DeFi tokens). The token mint authorization model described here is implemented by `promissory_note`'s `RegisterTypeV1` (token creation) and `IssueV1` (token minting). The `auth_mint_v1` and `auth_mint_freeze_v1` functions described below are not yet implemented in DarkWow.

## Current Situation

`Money::token_mint_v1()` (now `promissory_note::IssueV1`) allows minting a given token with the token ID
