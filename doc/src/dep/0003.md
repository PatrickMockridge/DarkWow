...
We now define `Money::token_mint_v1()`. Let the params be commitments $𝐂 = (Cᵢ)$
and `auth_parent`.
For each commitment $Cᵢ$, let there be corresponding proofs $πᵢ$ such that

**Token ID integrity** &emsp; $T$ is calculated correctly committing to
`auth_parent`.

**Commitment commitment integrity** &emsp; $Cᵢ = \t{PoseidonHash}(…, T, …)$

Additionally the contract checks that `auth_parent` is the function ID of
the parent caller.

The sole purpose of this call is to create a set of commitments whose token
...
* Constructs a pedersen commit $V$ to the value in the commitment, along
