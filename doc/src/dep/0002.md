...
owned liquidity in DarkWow. Without this, in order to have on chain DAO
treasuries, the DarkWow blockchain would have to recognize funds held by
...
mint phase of creating commitments, we construct a commitment
`C = hash(…, spend_hook, user_data)`. The `…` contains commitment data such
...
liquidity, we introduced the commitment attributes `spend_hook` and
...
When a commitment is spent, the `spend_hook` is revealed publicly. The
...
When spending a commitment where the `spend_hook = DAO`, then
