...
|----------------------|---------------------|------------------------------------------------------------------------------------------------------------------------|
| `u32`                | version             | Identifies protocol version being used by the node. **Implementation note**: The actual code uses `semver::Version` rather than `u32`. |
| `u64`                | timestamp           | UNIX timestamp                                                                                                         |
| `u16`                | nonce               | Random nonce, randomly generated everytime a version packet is sent. This nonce is used to detect connections to self. **Implementation note**: The `nonce` field is not present in the current `VersionMessage` struct; self-connection detection is handled via P2P key exchange instead. |
...
| `Vec<String>`        | ext_send_addr       | (Optional) External address of the node sending this message. **Implementation note**: The actual code uses `Vec<Url>` for typed-parsing safety. |
