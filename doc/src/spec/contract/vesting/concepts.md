...
* **Withdraw:** vestee can withdraw some amount of their vested commitment.
* **Forfeit:** vesting authority forfeits a specific vesting.
* **Exec:** overlay function over `Withdraw` and `Forfeit`, used as the
  spend hook binding the commitments to these contract calls.

## Vest

Vesting authority submits a vesting configuration on-chain, along with
a chained transfer call burning the to-be-vested input commitments, which
must be for the same token, and minting a new commitment with their total
...
> Note: There is a limitation though; the vested commitment can only be
> controlled by a single address. To overcome this, when the vesting
> authority creates the vested commitment, instead of using the recipients
> actual address, it generates a new random one, which is shared with
> the vestee(in a safe off-chain manner), so the commitment can be controlled
> by both parties.

> Note: Since vesting requires a 1-1 vested commitment to config matching, it
> means that those commitments can be tracked by the vesting configuration
...
their vested commitment, which is done by a chained transfer call burning the
existing vested commitment and creating two output commitments; one which will be a
normal commitment for a withdrawal address, and a second one representing the
...
represents the remaining balance vested commitment. We don't care if that
becomes a zero value commitment, since it won't be usable anymore, and we
...
> remaining balance commitment becomes a zero value one, but someone tracking
...
Let $Bv \ in [0, 2^{64})$ be the burned commitment value.
...
$$ \text{Withdrawn} = T - Bv $$
$$ \text{WithdrawCommitmentValue} = \text{Available} - \text{Withdrawn} $$
$$ \text{VestingChangeValue} = T - (\text{Withdrawn} + \text{WithdrawCommitmentValue}) $$
...
The burned vested commitment has value $Bv$ which represents the remaining
balance in the vested commitment. Initially (right after vest) $Bv = T$.
...
$$ \text{WithdrawCommitmentValue} = \text{Available} - (T - Bv) = \text{Available} - T + Bv $$
$$ \text{VestingChangeValue} = Bv - \text{WithdrawCommitmentValue} $$
...
First withdrawal at $t = 12$ with $Bv = 100$ as the initial vested commitment:

$$ \text{Available} = (12 - 10) * 90 + 100 = 280 $$
$$ \text{Withdrawn} = 1000 - 1000 = 0 $$
$$ \text{WithdrawCommitmentValue} = 280 - 0 = 280 $$
$$ \text{VestingChangeValue} = 1000 - 280 = 720 $$
$$ \text{Conservation: } 280 + 720 = 1000 = Bv $$

Second withdrawal at $t = 15$ with $Bv = 720$ from previous change commitment:

$$ \text{Available} = (15 - 10) * 90 + 100 = 550 $$
$$ \text{Withdrawn} = 1000 - 720 = 280 $$
$$ \text{WithdrawCommitmentValue} = 550 - 280 = 270 $$
...
$$ \text{WithdrawCommitmentValue} = 1000 - 550 = 450 $$
...
$$ \text{WithdrawCommitmentValue} = Bv - (T - \text{Available}) = Bv - \text{VestingChangeValue} $$

which is just the difference between what the commitment held and what must
remain locked.
...
then derive
$$ \text{WithdrawCommitmentValue} = Bv - \text{VestingChangeValue} $$

Proof simplification:
$$ \text{VestingChangeValue} = \text{BaseSub}(T, \text{Available}) $$
$$ \text{WithdrawCommitmentValue} = \text{BaseSub}(Bv, \text{VestingChangeValue}) $$
...
balance vested commitment and mint a new one to a recipient address, using a
