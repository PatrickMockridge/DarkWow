...
#   dwowd (mining fullnode) → xmrig (solo RandomX) → dwow_wallet (wallet + contracts)
#
...
#
# Prerequisites:
#   - Built binaries: ./target/release/dwowd, ./target/release/dwow_wallet
...
# Check binaries
BINS_OK=true
for bin in dwowd dwow_wallet; do
...
if [ -n "$WALLET_ADDR" ]; then
    ...
    info "To spend mining rewards, run: dwow_wallet -n $NETWORK wallet import-secret <hex_secret>"
...
if [ -n "${BALANCE_VALUE:-}" ] && [ "$BALANCE_VALUE" != "0" ] && [ "$BALANCE_VALUE" != "0.00000000" ]; then
    ...
    if [ -n "$DEFAULT_ADDR" ]; then
        ...
        # dwow_wallet transfer <amount> <token> <addr>
...
if [ "$fail_count" -gt 0 ]; then
    ...
    echo
    echo "  Next steps:"
    echo "    - Register contracts: dwow_wallet -n $NETWORK contract register <name> <ContractId>"
    echo "    - List known contracts: dwow_wallet -n $NETWORK contract list"
    echo "    - Invoke contract: dwow_wallet -n $NETWORK contract invoke <ContractId> <function>"
    echo "    - Check status: dwow_wallet -n $NETWORK wallet balance"
    exit 0
fi
