#!/usr/bin/env bash
# L4: dex contract — wallet capability verification
...
echo "  Proposing swap..."; call_contract 1 "$CID" "ProposeSwap" --token-x DRKW --token-y DRKW --amount-x 100 --amount-y 100
wait_for_blocks 1; scan_wallet 1
ACTUAL=$(get_position 1); EXPECTED=$(oracle_fixture "$CONTRACT" "propose")
