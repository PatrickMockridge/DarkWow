...
phase_modules: Dict[str, Module] = {
    ...
    "phase_09_blocks": Module(
        ...
        desc="Phase 9: Verify each mining node independently produces blocks. Cross-node consensus is a protocol property verified by chain_validation_model.py.",
        depends_on=["output", "config", "helpers"],
        functions=[
            Function("phase_blocks", "phase_blocks",
                     desc="Phase 9: verify each node independently mines blocks, inspect PoW/anchor on node0",
                     reads=["MODE", "NODE0", "NATIVE_NODES", "FINALITY_CARIBINA_ENABLED",
                            "FINALITY_ENABLE_MONERO"],
                     calls=["info", "pass", "fail", "warn"], lines=221),
