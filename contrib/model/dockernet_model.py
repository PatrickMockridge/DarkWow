...
Key Management — Clean Separation of Concerns (2026-07-02):
  ...
  Miner key flow:
    open(section=NODE_NAME) → keys.toml declared identity (no auto-gen)
    → default_public_key() for coinbase
    → export_base58(0) for key backup (darkwow account export --keys --section)

  Wallet key flow:
    open(section="wallet-N") → keys.toml declared identity, derived on boot
    → secrets() for scanning / AEAD decryption

  Pipeline key sharing (testing only):
    wallet-1 DECLARES the same secret as node0 in keys.toml — the owner's
    declaration boundary, not an export|import pipe. keys.toml is generated by
    `darkwow account generate` (generate-keys.sh). No shell-level key
    manipulation — no xxd, no bs58, no mining_secret file.
