...
/-! ===== The ZK-to-type bridge: DISCHARGED, by relocation and with a recorded strength change

    ASSUMED UNTIL 2026-09-24: for a resource/action pair, every public input of the corresponding
    Halo2 circuit is `constrain_instance`-derived from a witness — no free instances. It was an
    *uninterpreted predicate*: unlike the axiom it replaced (`circuitSoundnessBridge`), it asserted
    nothing about any particular resource or action — it named the premise so callers could supply it.

    **WHAT REPLACES IT.** `Capability.Inversion.CircuitDerivable` now carries the *data* — the names
    the circuit holds and its statement list, transcribed from the sources — and a field
    `noFreeInstances : Circuits.InstanceDerivation.DisclosureRule held stmts`: a computation over
    that data rather than an uninterpreted `Prop`. `proofs/lean/src/CircuitIndex.lean`, generated by
    `scripts/gen_circuit_index.py`, freshness-gated beside the transcription's and built as the third
    Lean library, supplies one inhabitant per pair for **12** of the 14 pairs
    `Capability/Composition.lean` instantiates. The other 2 are recorded there as decisions rather
    than omissions: `nativeTokenTransferType` and `nativeTokenCoinbaseType` resolve to no circuit at
    all, because `native_token`'s manifest is the FYI document its own header says it is and its
    coinbase path is host-side.

    **THE STRENGTH CHANGE IS WHAT TO READ HERE, not the word "discharged".** The rule proved at those
    twelve pairs is the *checker's* (`script/circuit_instance_derivation.py`), which admits three
    classes the name above does not: an exposure the circuit pins elsewhere (`redundant`), one a
    reviewed host-side justification declares free (`declared-free`), and a determination whose
    determining side is a declared constant. So the property now proved is **weaker than this entry's
    name**, and a reader who takes the name literally is over-reading the result. What forced that is
    measured rather than argued: of the 178 transcribed circuits, **167 fail the strict reading**, and
    the generator decomposes them exactly — 154 `redundant`, 12 `declared-free`, 1 the constant
    boundary. All **12** of the pairs the capability layer instantiates fail the strict reading, every
    one of them for a `redundant` exposure, and **none of the 12 has a `declared-free` instance** —
    so no exception list stands behind these proofs, which is the opposite of the shape a weakened
    predicate usually takes. The stronger reading cannot be silently re-adopted: the model carries
    **both** predicates, `noFreeInstance_imp_disclosureRule` proves the new one weaker, and the
    strict one's refutations remain theorems.

    **WHAT THIS DOES NOT ESTABLISH**, stated rather than implied. Nothing about the deployed circuits:
    the transcription is source-faithful *as data*, and the checker's own caveats are inherited
    unchanged — it reads `.zk` source rather than the `.zk.bin` that is deployed, and it does not know
    what the opcodes mean. And the `redundant` admission buys only that the exposure adds no freedom,
    *not* that the disclosed witness is safe: it stays prover-chosen, and whether that matters is a
    property of the contract's entrypoint and not of the statement list. That obligation is `OBL-Z1`'s
    and nothing here discharges it.

    **THE HISTORY, KEPT BECAUSE EACH CORRECTION IS A MEASUREMENT.** The route this entry first named —
    the manual circuit audit, mechanised, `doc/src/arch/verification-hazop.md` OBL-Z1 — was wrong in
    two ways and both were recorded rather than edited away. First it said the audit was "currently
    unchecked", from a grep that searched for the *Python* file's name while the wrapper is
    *hyphenated*: one side of a two-sided pair read alone. The audit **is** mechanised
    (`script/circuit_instance_derivation.py`, the register's only MECHANIZED row) **and it is a
    gate** — `run_gate "circuit instance derivation"`. Second, and the deeper point: **even a passing
    audit would not have discharged this predicate**, because the audit checks circuit *sources* while
    the axiom was `(r, s)`-indexed, and what sits between them — `(r, s) ↦ the circuit source` — is a
    *transcription*, not a check. A Lean term cannot read a `.zk` file, so the discharge needed the
    transcription *supplied*: a generated, freshness-gated module, which is what `Transcribed.lean`
    is. The audit itself **exits 0** as of 2026-09-24 — `0` unclassified over 178 circuits and 880
    `constrain_instance` sites, where this entry said 11 over 7 circuits and the runner's own comment
    said 5; the register's `OBL-Z16` carries the count and the gate is the authority.

    `Transcribed.lean` (`src/Transcribed.lean`) is generated by `scripts/gen_circuit_transcription.py`
    from the `.zk` sources, freshness-gated by `run_gate "circuit transcription freshness (OBL-T7)"`,
    and carries one `List Stmt` per circuit with a per-circuit verdict the *kernel* closes by
    `decide`. It is **not** reached from `src/DarkFi.lean`: it is a library of its own
    (`lean_lib Transcribed`) because it is this tree's most expensive elaboration — 178 `decide`
    proofs over 2677 statements — and while it rode on `DarkFi`'s import graph a
    `LEAN_NUM_THREADS=4` build of that library exhausted this host's memory and froze it,
    2026-09-24; `CheckAxioms.lean` imports it by name, so all 178 verdicts are still walked.

    **`IF FALSE` is retired with the assumption** — a computation carries a value rather than a
    falsity condition, and the field it had read "NOTHING; an uninterpreted `Prop` is a name, not a
    claim", which was ELEV-26's point. The sentence that stood there remains worth keeping for a
    different reason: it said `capabilityType_of_circuitDerivable` "is proved from
    `CircuitDerivable.coversBarbs` alone and never touches this predicate, because type *existence* is
    purely combinatorial". That is true of the proof term and was **false of the budget** — the
    `@[axiom_budget 1]` it carried was correct, because **a structure with a `Prop` field whose type
    names an axiom carries that axiom in every projection, including the projections of its data
    fields** (`CircuitDerivable.primitives` and `.coversBarbs` each read 1 on their own, measured
    minimally in `Capability/Inversion.lean`'s header). With the axiom gone that contamination is
    gone with it, and the theorem's budget is **0** — measured, not predicted. Recorded as SILENT in
    `DarkFi.HAZOP.Elevated` ELEV-26 and as a budget-mechanism hazard in `DarkFi.HAZOP.High` HIGH-16. -/
