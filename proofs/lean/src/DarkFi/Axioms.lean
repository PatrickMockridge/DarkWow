...
/-- The ZK layer's obligation to the type layer.
    ...
    *transcription*, not a check. A Lean term cannot read a `.zk` file, so the discharge needs the
    ...
    the premise.
    **SUPPLIED 2026-09-24, and what it measures is a warning about this entry's own name.**
    `Transcribed.lean` (`src/Transcribed.lean`) is generated by `scripts/gen_circuit_transcription.py`
    from the `.zk` sources, freshness-gated (`run_gate "circuit transcription freshness (OBL-T7)"` at
    `scripts/run-all-tests.sh:115`), and carries one `List Stmt` per circuit with a per-circuit
    verdict the *kernel* closes by `decide`. It is **not** reached from `src/DarkFi.lean`: it is a
    library of its own (`lean_lib Transcribed`, built by the gate as `lake build DarkFi Transcribed`)
    because it is this tree's most expensive elaboration — 181 `decide` proofs over 2747 statements —
    and while it rode on `DarkFi`'s import graph a `LEAN_NUM_THREADS=4` build of that library
    exhausted this host's memory and froze it, 2026-09-24; `CheckAxioms.lean` imports it by name, so
    the 181 theorems below are still walked. Under the model's rule, **170 of 181 circuits do not
    meet this predicate** — the generator decomposes them by asking the checker what it made of the
    same exposure, and 155 of the 170 are the checker's `redundant` class, 11 its `declared-free`
    class, 3 its own `OBL-Z16` failures, and 1 the model's declared-constant boundary. So this
    predicate's *name* is a strict reading of the tree rather than a description of it; what the
    tree enforces is the checker's four-verdict rule, and a caller that supplies this hypothesis is
    supplying something stronger than the deployment. The transcription does not make the axiom
    false — it is uninterpreted, so nothing is assumed — but a future discharge has to decide which
    of the two rules it means, and `(r, s) ↦ the circuit source` is still not in the tree.
